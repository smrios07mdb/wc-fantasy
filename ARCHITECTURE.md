...
- **The names attachment (the SOLE read-model change).** `loadPlayoffs` attaches `managerNames: Record<managerId,string>` (a `manager.findMany` in the owner-bypass edge); `@app/recompute`/`buildPlayoffsView`/`PlayoffsViewCore` are **byte-untouched**. The screen needs field-wide names (survivor rows + guillotined avatars); the pure core stays name-free. See DECISIONS → the scoped read-model exception.
