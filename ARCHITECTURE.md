...
- **Mobile CSS (`@media max-width:720px`) — fixed-fit height + proportional token unit.** The pitch turns vertical (away top / home bottom, halfway split). `.gd-pitch` height is `min(540px, calc(100dvh - 232px))` — it shrinks with the REAL viewport (dvh minus the page chrome above + the fixed bottom nav) and is **a CAP, never a content floor**: `overflow:hidden` means it can never scroll or grow (the lane-wrapping `min-height:600px` floor is the bug this removes). Each `.gd-phalf` takes an equal slice (`flex:1; justify-content:space-evenly`); each `.gd-pline` is a horizontal row spread ACROSS the full width (`justify-content:space-evenly`, **`flex-wrap:nowrap`** — a flat 4 shrinks, never wraps). The token unit `--shirt = clamp(15px, min(height-budget, width-budget), 44px)` where height-budget `= (halfHeight / --pitch-rows) − text-strip` and width-budget `= (100vw − chrome) / --pitch-cols`; **take the smaller** (on a phone the height almost always binds). Every token element (rating square, status badge, name, fpts pill) scales proportionally off `--shirt`. Token size is the variable; legibility yields to the no-scroll fit.
...
- **Layout guard `apps/web/scripts/verify-pitch-layout.mjs` (`pnpm test:layout`).** OPT-IN (NOT in `pnpm test` — that stays browser-free for CI); SKIPS with exit 0 if no Chromium binary. jsdom has no layout engine, so a unit test can be green while the rendered pitch overflows/overlaps — this renders a faithful replica of the pitch DOM with the REAL `ds.css` + `games.css` in headless Chromium and asserts true `getBoundingClientRect()` bounds. For 6 formations (4-3-3, 4-2-3-1, 3-5-2, 4-4-2, 4-5-1, 3-4-3) at 360 / 390 (phones) + 1280 (desktop) it asserts: (a) the full pitch fits — content within the pitch box, box ≤ viewport, no scroll; (b) ZERO token overlap, including the rating-vs-sub-badge on the SAME token; (c) ZERO horizontal clip; (d) the right line structure per band. It also saves `/tmp/pitch_<formation>_<width>.png` for 4-2-3-1, 4-4-2, 3-4-3 at 360+390 (the mockup-review artifacts — rendered screenshots are part of the pre-merge pause, since the last rounds shipped green-but-broken).
