...
- **Layout guard `apps/web/scripts/verify-pitch-layout.mjs` (`pnpm test:layout`).** OPT-IN (NOT in `pnpm test` — that stays browser-free for CI); SKIPS with exit 0 if no Chromium binary. jsdom has no layout engine, so a unit test can be green while the rendered pitch overflows/overlaps — this renders a faithful replica of the FULL screen chain (shell content area → `.gd-app` → fixed scoreboard/stake/tabs → flex `.gd-tabwrap` → `.gd-lineups`, with a tall list stand-in so the tab actually scrolls) with the REAL `ds.css` + `shell.css` + `games.css` in headless Chromium, so the pitch flex-fills the real leftover exactly like the app, and asserts true `getBoundingClientRect()` bounds. For 6 formations (4-3-3, 4-2-3-1, 3-5-2, 4-4-2, 4-5-1, 3-4-3) at 360 AND 390 across a ROOMY (844) and a TIGHT (667, iPhone-SE class) viewport height — so the flex-fill is exercised in both regimes — plus 1280 (desktop), it asserts: (a) the full pitch fits — content within the pitch box, box ≤ viewport, no scroll — **at both heights**; (b) ZERO token overlap, including the rating-vs-sub-badge on the SAME token; (c) ZERO horizontal clip; (d) the right line structure per band. It saves `/tmp/pitch_<formation>_<w>x<h>.png` for 4-2-3-1, 4-4-2, 3-4-3 at 360 × {667, 844} (the mockup-review artifacts — rendered screenshots are part of the pre-merge pause, since the last rounds shipped green-but-broken).
