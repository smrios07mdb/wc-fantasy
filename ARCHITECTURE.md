...
**`dispatchCommissionerAlert` — a fourth, GOVERNANCE dispatch (additive, `feat/autofire-round-cut`).** A sibling of `dispatchToManager` that reuses the SAME Web Push transport + `notification_sent` UNIQUE-ledger claim but DROPS the preference gate: a commissioner adjudication nudge (the auto-fire tie-hold, free-TEXT kind `cut_needs_review`, at-most-once per round) is not an opt-out channel. `dispatchToManager` + the closed `NotificationKind` union + `KIND_TO_PREF` are byte-identical; the ledger `kind` column is free TEXT (no migration) and is typed by a widened `LedgerKind = NotificationKind | AlertKind` superset on `claimLedger` only. The auto-fire worker resolves the flagged-commissioner recipient(s) and calls it; it is invoked by nothing else.
...
- **`PlayoffAdvanceStore` (`advanceStore.ts`).** `loadRoundContext(leagueId, roundLabel)` assembles the knockout `period` (`cut_count`/`frozen_at`), the `alive` field with each manager's round score (`score_manager_period` for that period, 0 where absent) + **cumulative tournament total** (Σ `score_manager_period.points` over ALL the league's periods via the period relation — on the fly, no stored column; derived via the shared **`loadCumulativeTournamentTotals`**, see the single-source note below), plus `alreadyCut` (≥1 entry stamped `eliminated_round == roundLabel`) and `uncutPriorRounds` (the ordering guard). `applyRoundCut` flips `alive → eliminated` (+ `eliminated_round`/`eliminated_at`) and the lone survivor `alive → champion` in ONE `$transaction`, idempotent via a conditional `alive → eliminated` claim (0 rows ⇒ already cut). Pinned end-to-end by a gated real-Postgres suite (`advanceStore.integration.test.ts`, `PLAYOFF_PG_TEST_URL`). **Since `feat/commish-advance-release` (main `c37001b`, 2026-07-03) the SAME `$transaction` ALSO releases the just-cut managers' ENTIRE active roster to the free-agent wire** via `@app/faab/prisma`'s `releaseEliminatedRosters(tx, …)`, which reuses the shared `releaseRoster` body (`runReleaseInTx`) through an OPTIONAL `tx` param that ENLISTS in the outer transaction — the **optional-tx enlistment pattern**: `tx` present ⇒ join the caller's tx, absent ⇒ self-open one (behavior-identical to before). The `tx` param rides the CONCRETE adapter only, NOT the pure `FaabReleaseStore` port (kept `@app/db`-free per `purity.test.ts`), so the manager route + `commish:trim` are unchanged. Locked slots are released under the `app.commish_override` GUC scoped to the cut round (`periodId: round.id`) — deliberately clearing the eliminated managers' final-round lineup snapshot (frozen scores/standings unaffected); the idempotency claim fronts the release (a re-run releases nothing); the champion + previously-eliminated managers are never released.
- **`runRoundAdvance` (`advance.ts`, orchestrator).** Dry-run default; guards = commissioner gate, required reason, real round label, frozen precondition (`--allow-incomplete` override), ordering guard, champion/schedule sanity checks. A residual tie is surfaced in the dry-run and refused on `--apply` unless a valid `--break-tie`. One audit line per applied cut + the champion flip — each now carrying the `released` player ids (the roster shed to the wire); the dry-run plan additionally enumerates `releasePreview` (the per-cut-manager blast radius, via the store's `loadActiveRosters`), surfaced in the CLI report + the `/commish` type-to-confirm.
...
- **First-open cut ceremony (`feat/cut-ceremony-first-open`, 2026-07-04) — the SHARED per-device seen-latch.** The Chocoyo blade (here) and the `/vsfield` "The Cut" `KOCeremony` takeover (§27) both fired ONLY when a client watched a round flip `live→past` between refetches; a late-opener missed the reveal. **`apps/web/src/playoffs/seenCeremony.ts`** is the single shared utility both surfaces import: a per-device localStorage seen-set keyed **`xi:seenCut:<leagueId>:<roundLabel>`** (`seenCutKey`), storage-safe `hasSeenCut`/`recordCutSeen` (SSR-safe + quota/blocked-store **fail-toward-seen** so a takeover never replay-spams), and the pure `decideCeremonyLatch` (fire the most-recent past cut IFF unseen; `record` = ALL past labels; `SEED_SILENT_DEFAULT=false` ⇒ fire-latest-once). **Per-surface MOUNT EFFECT (client-only, mount-once — never SSR/render, so no hydration mismatch):** diff the PAST cut rounds the view-model already exposes against the seen-set and fire once (Theater: play the blade for the unseen round's idx; The Cut: `setCeremonyOpen`), then record all. It is **DISJOINT from the existing live→past latch** (on mount `prev=null` ⇒ the live latch no-ops), and BOTH now `recordCutSeen` into the SAME key — so a fire on either path OR the other surface suppresses every later replay (the **shared-key** model: cross-surface no-replay). Reduced-motion records-but-static; champion/complete is a PERSISTENT resting state, out of the one-shot latch. `loadPlayoffs` stays the CLOCKLESS STOP seam — byte-untouched; `leagueId` (absent from `PlayoffsView`) is threaded to `PlayoffsClient` via a PARALLEL indexed read in `page.tsx`, not by extending the loader. Render proofs: `verify-the-cut.mjs`/`verify-playoffs-hero.mjs` both gained a real-browser localStorage-gated latch proof (first-open-fires+writes / second-open-settles / cross-surface / reduced-motion, desktop+mobile); `ceremonyLatch.dom.test.tsx` mounts the REAL clients; `seenCeremony.test.ts` pins the key string + decision. See DECISIONS.md → 2026-07-04.
...
**The Cut (T15-CUT, 2026-07-03) — the unified-knockout composition on the SAME seam.** `/vsfield` is the one knockout surface now ("The Cut"); the §27 hide and the reference's show-the-fallen reconcile as **one ladder, two sections**, composed at the same loader seam with every engine byte-untouched: `filterEliminatedFromField` stays byte-identical for the ALIVE ladder, and a new additive `ko` sibling (`VsFieldViewWithBenches.ko`) carries the guillotine framing. `loadVsField` adds the loadPlayoffs-shaped reads — `playoff_entry` (managerId/seed/status/eliminatedRound) on wave 1, `period.cutCount` on the existing periods select, knockout `score_manager_period` + the SHARED `loadCumulativeTournamentTotals` on wave 2 — ALL gated on playoff_entry existence (group phase: no extra read, `ko` undefined; pinned pure + DOM-level, rider E). The pure `buildKnockoutContext` (`apps/web/src/vsfield/knockout.ts`) projects the untouched `buildPlayoffsView`'s live round — the tie-widened provisional zone from the SAME `resolveRoundCut` the `commish:advance` apply path calls, so the displayed blade can never drift from the eventual cut — over the PRE-filter `buildVsField` field (fallen identities/pts come from the unfiltered engine output; past-round `ranked` rows supply pts-at-cut; group-phase non-advancers surface as "Group stage" fallen via the no-row contract above). `ko` attaches only on the live knockout wave (live/pend arms) or the complete default view (champion arm — there may be no live period post-Final); a selected prior stays a plain T11 view, which is also the fallen drill-in path (`?period=<cutRound>&manager=<id>`). **Realtime:** the vsfield channel gains a knockout-only `playoffEntryBinding(leagueId)` — LEAGUE-FILTERED (`league_id=eq.<id>`) by contract, deliberately NOT the /playoffs channel's unfiltered shape — so a round-advance nudges the same snapshot refetch that feeds the ceremony latch (live→past over `ko.roundStatuses`, the Chocoyo pattern) and the fallen section; no publication/RLS change (the table was already published for /playoffs). **F-P2-K3 closed in `startVsFieldLive`:** injected visibility wiring refetches immediately on tab resume and reports `onStale` when the last applied snapshot is over `STALE_AFTER_MS` — the ConnPill "Delayed" branch is live. `/playoffs` is the ceremonial Theater (hero/choreography/champion/OnTheBlock only; `loadPlayoffs` byte-identical — its `reducedLineup`/`reinforcement` pass-throughs are now UI-unused, a recorded future trim). See DECISIONS.md → 2026-07-03 (T15-CUT).

## §28 — Commissioner console (`/commish`) + the `commish_audit` ledger + the `recordCommishAudit` write seam (`feat/commish-shell`, Thread 1)

**What.** The first commissioner-only PAGE, plus the cross-cutting audit table every later commissioner write slice writes into. Thread 1 is read-heavy: the table ships EMPTY, no write path is wired. Threads 2–5 layer the Tier-1 writes → roster/lineup repair → freeze → field-lock on top, each recording into `commish_audit`.

### The `commish_audit` table (the migration — `20260701120000_commish_audit`)
Additive `CommishAudit` model at the schema tail (alongside `NotificationSent`, the other server-write-only ledger). Columns: `id`, `league_id` (FK→`league` Cascade), `actor_user_id` (FK→`app_user` SetNull, **nullable** — a future 'system'/automated row carries a null actor), `action_type` (TEXT), `summary`, `detail?`, `reason?`, `target_ref` (JSONB? — a structured pointer `{matchId,playerId}` | `{managerId}` | `{periodId}`), `delta?` (display string, e.g. `+5 pts`), `reversible` (bool default false), `reversed_at?`, `reversed_by_user_id` (FK→`app_user` SetNull?), `created_at`. Index `(league_id, created_at)` (plain btree; a backward scan serves `ORDER BY created_at DESC` — the repo uses no DESC index keys).

- **`action_type` is app-union, not a pg enum** (DECISIONS §2): free TEXT; the closed set is `COMMISH_ACTION_TYPES`/`CommishActionType` in `packages/shared/src/commish.ts`, so a new slice needs no migration.
- **RLS** (the migration's hand-authored block, after the `migrate diff` DDL): ENABLE (not FORCE) → the owner client bypasses; **one** `FOR SELECT TO authenticated` commissioner-only policy (`m.league_id = commish_audit.league_id AND m.user_id = (auth.uid())::text AND m.is_commissioner`, reading the caller's own `manager` row via `manager_select_own` — **no** SECURITY DEFINER helper, since the table carries its own `league_id`); **zero** write policies → client INSERT/UPDATE/DELETE default-denied; **not** in `supabase_realtime`. Both portability shims (`authenticated` role + `auth.uid()`) copied from `watchlist`/`pool_pick`. RLS is defense-in-depth on the Data API surface — the app's real read is the owner-bypass loader below.

### The `recordCommishAudit` write seam (`apps/web/src/commish/recordCommishAudit.ts`) — STUB, NO callers this thread
The single server-side writer every later slice calls (inside its mutation transaction, so the audit row and the effect commit atomically). It inserts via the Prisma owner client (RLS-bypass — the ONLY write path). The insert is injected (defaults to `prisma.commishAudit.create`) so the field mapping is unit-testable DB-free (the injected-store idiom, cf. `handleStartDraft`); `target_ref` is spread-OMITTED when null (a Prisma `Json?` column rejects a literal JS `null`). **Thread 1 wires ZERO callers** — the table ships empty; grep-confirmed the only references are its own file + tests.

### The read path — `loadCommish` (owner-bypass) + the pure view (`commishView.ts`)
`apps/web/app/commish/loadCommish.ts` reads through the RLS-bypassing owner client (like every web loader) and shapes props via pure helpers. It gathers (one `Promise.all`): league name, the league's managers (id/displayName/isCommissioner/faabBudget), period + frozen-period + audit-entry counts (System-status card), and the recent audit rows (ordered `createdAt desc`, capped 50, with the `actor` relation). `commishView.ts` (pure, tested) shapes each audit row (`toAuditView` — server-computes a relative `whenLabel` so there's no client `Date.now()` hydration mismatch) and the inspector (`toInspector`).

### The gate + the view-as inspector
- **Gate:** `resolveCommishAccess` (`commishGate.ts`, pure) mirrors `api/draft/timer` — no-session → `/sign-in`, non-`ok` → `/auth/denied`, non-commissioner → `/auth/denied`, commissioner → ok. "Commissioner" is the shared `resolveCommissioner` (`@app/shared`) = `is_commissioner` flag OR the `smrios07@gmail.com` email from the validated JWT — the SAME predicate the worker CLI uses (the worker's `isCommissionerActor` now delegates to it; DECISIONS §4). `page.tsx` runs the gate BEFORE `loadCommish` (redirect() throws → the loader is unreachable for a denied viewer).
- **View-as (read-only inspector, NOT impersonation; DECISIONS §5):** `?as=<managerId>` → `loadCommish`'s `buildInspector` reuses `loadStandings` (record + seed — pick the target's row from the whole-league cumulative), `manager.faabBudget`, and a contained `rosterPlayer.findMany` (owned players). **Same-league-only:** the `managers.find` guard (over the leagueId-filtered manager set) runs BEFORE the roster/standings read, so an out-of-league `?as=` yields no inspector. No token mint, no identity swap, no mutation; the switcher navigates via a GET `<Link>`.

### The UI + the gated nav
`CommishConsole.tsx` (one responsive client component) + route-scoped `commish.css` port `design/design_reference/admin` (the `adm-*` family; the dead `components2.jsx` `cm-*` family ignored). The slate "elevated privileges" identity rides `--adm-edge = var(--locked)` (the one admin-local token; all others are global in `ds.css`); cobalt = tab selection + view-as action, warning = `--ytp`, never gold. The four task tabs render as **inert placeholders**; the two live read-only pieces are the Audit-log panel (real rows, empty-state until slices populate) and the view-as `ManagerView`. **Nav:** `commish` is a new `NavId` (+ an exhaustive-glyph key), but `COMMISH_NAV_ITEM` is kept OUT of the always-rendered lists and rendered only when the new optional `AppShell`/`MoreSheet` `isCommissioner` prop is truthy (threaded by the hub page + the `/commish` layout; global surfacing is a documented follow-up seam — DECISIONS §6).
...
**Transient-throw gap → the saved-but-restate-pending response shape (`fix/commish-tier1-writes`, additive).** The write + audit `$transaction` commits BEFORE the synchronous `deps.rescore(...)` fires. If that re-score THROWS (notably the frozen-override `recomputeManagerPeriod(..., {allowFrozen:true})` rollup), the correction is durable but the leaderboard is not yet restated — and the naked throw would propagate out of the handler and surface as a bare **500**, hiding the persisted write and inviting a duplicate re-entry. `handleStatCorrection.ts` now wraps the post-commit call in `fireRescore(...)`: a throw is caught and converted to a distinguishable **200** partial-success carrying `restatePending: true` + `warning: "restate_pending"` + a `message` (mirroring the existing `no_match_participation` 200 shape; `outcomeFields(scored, restatePending)` gives restate-pending precedence so the two warnings never co-appear, and `scored` stays `false`). This stays DISTINCT from a **pre-write validation reject**, which persists nothing and returns a clean 4xx `{ error }`. **Remedy = re-submit the identical correction:** the writes are absolute + idempotent (SET, never accumulate — see the idempotent-SET proof), so a repeat is safe; and for a FROZEN period it is the ONLY restate path — the worker sweep runs WITHOUT `allowFrozen`, so `dirty=true` alone re-scores `score_player_match` but leaves the frozen manager-period rollup (the leaderboard) stale until the write path itself restates again. Pinned by two handler unit tests (penalty + rating, throwing rescore → 200 signal, write+audit still recorded) + a gated-PG assertion (frozen period, real store, injected throwing rescore → manual row + audit DURABLE, response `restatePending`, NOT a 500). See DECISIONS.md → 2026-07-01 (transient re-score throw → saved-but-restate-pending).

**The audit call sites.** `recordCommishAudit` is now WIRED (Thread 1 shipped it with zero callers): two callers, both inside `applyPenalty`/`applyRating`'s transaction. `action_type` = `penalty_applied` (the pre-seeded union value, NOT the prompt's prose `penalty_entry` — DECISIONS §3) / `rating_override`; `target_ref = {matchId, playerId}`; `reversible=true`; `delta` = "+2/−2 pts" / "rating N". **Frozen-override surfacing:** the correction's past-freeze fact is recorded in the audit `detail` field (NOT a new boolean; `target_ref` stays `{matchId, playerId}`) — `handleStatCorrection.ts` sets `detail: periodFrozen ? FROZEN_NOTE : null`, where `FROZEN_NOTE` = "Applied to a FROZEN period — commissioner override re-scored the result past the freeze gate." The audit ledger renders it per entry (`CommishConsole.tsx` `AuditEntry` → `{entry.detail && <span className="adm-audit-detail">…}`), so a reviewer sees which corrections bypassed `result_freeze_hours`; the pre-write frozen banner warns before the fact.
...
**Invariants + gate.** Engine byte-untouched (empty `git diff --stat` on `packages/scoring` + `packages/recompute`). Gate green (typecheck/lint/format/**2878 passed / 74 skipped**/`@app/web` build, `/commish` `ƒ` + 2 API routes) + gated-PG on throwaway `postgres:16`: `commishAuditRls` 10/10 (incl. PART-A confirm-1) + `commishStatWrite.integration.test.ts` 5/5 (own `COMMISH_WRITE_PG_TEST_URL` + SAFE latch, distinct from the audit suite's URL so they never co-run) — the 4th is the **idempotent-SET re-score proof**: a re-submitted `penalty_won=1` re-scores to the SAME +2 over baseline (never +4), one manual row, one audit per write, exercising the real `createCommishRescore` on Postgres; the 5th is the **transient-throw proof**: a frozen correction whose post-commit re-score throws leaves the manual row + audit DURABLE and returns the `restatePending` signal (not a 500). **Source-only → MERGED** (`9da186e`, verified ancestor of `origin/main`). See PROJECT.md → 2026-07-01 (Commissioner console Thread 2) + DECISIONS.md → 2026-07-01 (Commissioner console Thread 2) + BACKLOG.md → Commissioner console.

### §28.3 — Thread 2b: general stat-line editor (`feat/commish-stat-editor`, source-only)

`POST /api/commish/stat` — a commissioner overlay for ANY of the **23 scoring-relevant** raw feed stats of one (match, player), stored sparsely in `manual_stat_player_match.extra.statOverrides`. It generalizes §28.2's write shape to N fields with no migration: the `extra` `jsonb` column already exists (⚠️ DISTINCT from the FEED `stat_player_match.extra`, a different column on a different table — never conflate).

**The adapter overlay seam (the ONLY `packages/recompute` edit; `packages/scoring` byte-untouched).** `adapter.ts` gains `OVERRIDABLE_STAT_KEYS` (the 23 — the single source of truth for the allowlist), `parseStatOverrides(extra)` (a bounded, defensive read of `extra.statOverrides`), and `ManualRow.statOverrides`. `buildScoreInput` merges per allowed field with `n(overlay[f] ?? feed[f])` — nullish, so an explicit 0 wins; the feed passes through where unset; the 4 inert denominators keep reading the raw feed. `prismaStore.getPlayerMatchInput` parses `manual.extra` into `ManualRow.statOverrides`, alongside the existing `roleFrom` read of the same `extra`.

**Gate isolation (the load-bearing invariant).** The overlay is carried on `ManualRow` and merged ONLY inside `buildScoreInput` — NEVER folded into `b.stat`. `playerAppearedInMatch` gates on `statHasData(b.stat)` and never inspects `b.manual`, so a manual-only correction on a feed non-participant stays `scored:false`/pending (§28.2 semantics) and folds in once the feed records the player. Proven at the unit level (`adapter.test.ts`) and on real Postgres (`commishStatWrite` gate-isolation case).

**Allowlist integrity = 23, structurally.** The engine scores only the `completed`/`won`/`accurate` companions, so `dribblesAttempted`/`duelsLost`/`passesTotal`/`longBallsTotal` are carried into `ScoreInput` but never read — excluded from the allowlist (overriding them is a points no-op). `adapter.test.ts` probes every `StatRow` field (bump 0→BIG per role — does `scorePlayerMatch`'s total move?) and asserts the scored set EQUALS `OVERRIDABLE_STAT_KEYS`; a `Record<keyof StatRow, true>` literal forces a classification decision if a 28th field is ever added, so "23" can never drift silently.

**Write + audit + UI.** `commishStatStore.applyStatCorrection` read-modify-writes `extra.statOverrides` (preserving `rolePlayed` + the penalty columns; empty → drop the sub-key → SQL NULL via `Prisma.DbNull`), sets `dirty:true`, and records ONE `commish_audit` row (`action_type=stat_correction`, `delta`=raw field-change string like `goals feed→2 · assists 1→feed`) in one `$transaction`; the sync re-score + `allowFrozen` + `restate_pending` path is reused verbatim. The write boundary (`validateStatOverrides`, pure) REJECTS unknown keys (`unknown_stat_key`) and non-integer/negative values (`bad_request` — `n()` does not clamp). `CommishConsole.tsx`'s `StatLineEditor` replaces the `// TODO(2b)` marker: a grouped field editor (current feed value + override input per field, role-gated dimming, clear-all), fed by `loadCommish`'s extended `current` (`feedStats` + `statOverrides` + `hasStatRow`).

**Invariants + gate.** `packages/scoring` diff EMPTY; the only `packages/recompute` edit is the additive `adapter.ts` overlay + the `prismaStore.ts` parse. Gate green (typecheck/lint/format/**2920 passed / 74 skipped**/`@app/web` build, `/api/commish/stat` in the route manifest) + gated-PG `commishStatWrite` **8/8** (+3 2b: multi-field SET re-scores + idempotent, rolePlayed-survives-clear, gate-isolation on real Postgres). **Source-only → MERGED** (`db2a2bc`, verified ancestor of `origin/main`). See design/COMMISH_2B_stat_editor.md + PROJECT.md / DECISIONS.md → 2026-07-01 (Thread 2b) + BACKLOG.md → Commissioner console.

### §28.4 — Thread 3a: SAFE roster/lineup repairs + the `@app/commish-core` extraction (`feat/commish-roster-repair`, source-only)

**The reuse seam (cleared Option 1).** The worker's DI commissioner-override orchestrators — `runRosterOverride` / `runLineupOverride` / `runTrimOverride` + the `core.ts` helpers (`isCommissionerActor`, `kickoffGuard`, `rosterEndStateHolds`/`lineupEndStateHolds`, `relaxPeriodLock`, `formatAudit`, name resolution) — were RELOCATED VERBATIM from `apps/worker/src/commish/` into **`packages/commish-core` (`@app/commish-core`)**: seven `R100` renames + package scaffolding only (`193193a`). The worker CLI (`cli.ts`) and the web repair handlers now import the SAME runners; `advance.ts`/`transition.ts` keep importing the core helpers from the package. Validation is never re-derived: the runners call `validateFaGrant`/`validateRelease`/`validateLineup` and the mutation primitives `claimFreeAgent`/`releaseRoster`/`saveLineup` as-is.

**The web surface.** `POST /api/commish/roster` (`kind:"add"` — add / add-drop with an optional SAFE period pin; `kind:"trim"` — drop-only multi-release) + `POST /api/commish/lineup` (XI for a period, past-window via `relaxPeriodLock`). Thin routes → pure `handleRosterRepair.ts` handlers (the Thread-2 `gate()` — now exported from `handleStatCorrection.ts`, +`email` on `GatePass` — 401-before-403 before any read; reason-required; semantic 404s) → the runner over `commishRepairStore.ts` (the live `@app/faab`/`@app/lineup` prisma stores passed through verbatim + a CLI-parity `getAddMatch`). Runner statuses map: `planned`→200 dry-run (the UI's Preview), `skipped`→200 idempotent (NO audit row), `refused`/`conflict`/`blocked`→409 with the runner's own reason (lock-class codes get a note naming the deferred 3b/CLI path), `applied`→ post-mutation audit + restate.

**The 3a safety contract (load-bearing).** `allowPostKickoff:false` / `allowLocked:false` / `allowLockedSlot:false` are hardcoded literals in the handler — not body fields — so the `app.commish_override` GUC (set only inside the two primitives when those flags are true) is unreachable from the web; the kickoff guard and the lock-on-play latch (validator phase-4 + the store's own latch re-check + the DB trigger) stay fully armed. Proven RED-first: unit (smuggled-flag tests) + gated-PG (`commishRepair.integration.test.ts`) with a trigger-armed canary and a locked BYSTANDER slot immovable through a successful 3a repair.

**Audit + restate seams.** Audit is POST-MUTATION in its own tx (the primitives own their `$transaction`s and accept no injected insert — B4): `roster_repair` (trim reuses it, `detail` `trim:true …`) / `lineup_repair`, `targetRef {managerId}` / `{managerId, periodId}`, `reversible:true`; a failed audit write → 200 `audit_pending` + the COMPLETE payload (never silent). Restate = `createCommishRestate` (`recomputeManagerPeriod {allowFrozen:true}` + `markManagerPeriodProcessed` per period + `recomputeStanding` once — public `@app/recompute` orchestration only; NOT `createCommishRescore`, which is a per-(match,player) stat re-score); scope: lineup = the edited period, roster/trim = not-closed periods ∪ the pin (B3 conservative). A throw → `restate_pending`.

**UI.** `CommishConsole.tsx` gains the **Roster & lineup** tab (`?tab=repair&rmanager=&rperiod=`, `loadCommish.buildRepair` — owner-bypass, same-league guard): live-unowned pool picker (eliminated-team players stay listed — the runner passes `allowEliminated`, the deliberate D2-safe bypass), unlocked drop select, period pin, trim checklist vs the phase cap (`rosterCapForPlayoffPhase` over playoff-entry existence), XI checklist prefilled from the current starters. Dry-run-first everywhere; NO dangerous-bypass controls rendered. See PROJECT.md / DECISIONS.md → 2026-07-02 (Thread 3a) + design/COMMISH_3_roster_lineup_repair.md.

### §28.5 — Thread 4: period freeze / unfreeze (`feat/commish-freeze`, source-only)

**What `frozen_at` actually gates (the copy contract).** `period.frozen_at` gates AUTO-RESTATEMENT ONLY: the sweep's Phase 2 skips a frozen period's `manager_period` markers unless `allowFrozen` (`recompute.ts:99` — the marker is left UNPROCESSED, `skippedFrozen`). It does NOT lock lineups (lock-on-play latch) and does NOT pause scoring (`period.status`). The design prototype's "Lineups locked · scoring paused" copy was WRONG and did not ship — console copy states the real effect: FREEZE = results final now (late feed/rating corrections stop auto-restating); UNFREEZE = re-open auto-restatement (pending markers restate on the worker's next sweep).

**The write surface.** `POST /api/commish/freeze` (stamp `frozen_at = now` — early-finalize before the 6h window, or re-freeze after a manual unfreeze) + `POST /api/commish/unfreeze` (`frozen_at = NULL`, reason REQUIRED, response surfaces `pendingDirty` — the unprocessed `manager_period` markers a next sweep will restate — + the re-freeze warning). Thin routes → pure `handleFreeze.ts` handlers (the shared Thread-2 `gate()`, 401/403 before any read; reason-required; same-404 for unknown AND cross-league periods — no existence leak) → `commishFreezeStore.ts`. Guard: freezable = status-closed OR (≥1 fixture ∧ all `completed`) — never a live/future wave; anomalous waves (postponed/abandoned fixture) stay unfreezable here, same as the cron. The `periodFreezable`/`periodLive` predicates are SHARED between the handler and `loadCommish.buildOps`, so the console's button state and the server guard cannot disagree.

**Idempotency + the cron race.** Already-frozen freeze / not-frozen unfreeze = typed 409 (`already_frozen`/`not_frozen`), NEVER a silent 200: a 200 without an audit row would break "every action is logged", and a duplicate audit row would log a no-op as an action. The store's write is a conditional `updateMany` (`WHERE frozen_at IS NULL` / `IS NOT NULL`) + the `commish_audit` insert in ONE `$transaction` — count 0 (a concurrent hourly-cron stamp or double-submit) returns null with NO write and NO audit row → the same typed 409. Audit rows: `period_freeze`/`period_unfreeze` (already in the free-TEXT union — no shared edit), `targetRef {periodId}`, `reversible:true`.

**Writers of `frozen_at` are now exactly two:** the periodClose cron (`periodClose.ts:104`, unchanged) and this surface. The cron's `frozenAt: null` query has NO manual-unfreeze exclusion, so an unfrozen settled period RE-FREEZES on the next hourly pass (~1h correction window) — surfaced in the unfreeze response, the audit detail, and the panel copy. No restate trigger fires on unfreeze: the ≤60s worker sweep picks up the still-pending markers itself (freeze-skip leaves them claimable — the deliberate Phase-2 design).

**UI.** The Game-operations tab goes live (`CommishConsole.tsx` `OpsPanel`, fed by `loadCommish.buildOps`): per-period rows (kind · status · frozen-since · pending-corrections-held count · live pill), type-to-confirm FREEZE (the design's confirmWord pattern, corrected copy) and plain-confirm unfreeze (reason required). packages/scoring + packages/recompute + the periodClose cron BYTE-UNTOUCHED (new callers of the existing gate only). See PROJECT.md / DECISIONS.md → 2026-07-02 (Thread 4).

### §28.6 — Thread 5: playoff round-advance (`feat/commish-advance`, source-only)

**The orchestrator lives in `@app/commish-core` now.** `advance.ts` (runRoundAdvance — front guards, idempotency skip, seeded-cut-count + ordering + frozen preconditions, pure `resolveRoundCut` tie handling, champion/schedule sanity, transactional apply) and `advanceStore.ts` (the `PlayoffAdvanceStore` port + Prisma adapter + memory double) relocated VERBATIM from `apps/worker/src/commish/` (pure git renames, pre/post hashes identical — the `193193a` Thread-3 precedent). The worker CLI (`commish:advance`) and all worker advance tests re-point imports only. The orchestrator rides the root `@app/commish-core` export; the Prisma adapter + memory double ride the new `./advanceStore` subpath export so the root export graph stays free of the `@app/db` runtime import (the `@app/faab/prisma` pattern). commish-core gains `@app/db` + `@app/recompute` deps.

**The web surface is a CALLER, never a re-derivation.** `POST /api/commish/advance` (thin route) → `handleAdvance.ts` (pure; shared `gate()` 401/403 before any read; 400 `unknown_round`/`reason_required` before any read) → `runRoundAdvance` over `commishAdvanceStore.forAdvance`. Reads delegate to the relocated Prisma adapter verbatim (one round-context assembly). The WRITE re-owns the transaction — Prisma interactive transactions don't nest, so the web adapter mirrors the relocated adapter's two conditional `updateMany` claims exactly (alive→eliminated entry gate, 0 rows ⇒ "already-cut" no-op; alive→champion flip) and adds exactly ONE `commish_audit` insert (`round_advance`, `reversible:false`, tx-bound `recordCommishAudit`) — the Thread-4 freeze-store atomicity precedent, pinned against the verbatim adapter by the gated-PG suite. A lost idempotency race writes NO ledger row.

**Web-surface hardening.** `allowIncomplete` (the CLI's frozen-precondition emergency override) is PINNED false: `parseAdvanceBody` never reads the field, the handler hardcodes `allowIncomplete: false` (the 3a smuggled-flags precedent, test-pinned). Status→HTTP: planned/applied → 200; skipped/needs-commissioner/refused → typed 409 with the discriminated `status` + plan (never a silent 200 masking a refusal — the Thread-4 typed-409 rule). Dry-run reason: apply:false synthesizes the fixed `ADVANCE_PREVIEW_REASON` so the panel's initial render never demands prose for a persist-nothing read; the real reason gates apply:true only. The orchestrator stays byte-verbatim.

**UI (Playoff-cuts tab, replaces the inert "Playoff field" placeholder).** `loadCommish.buildAdvance` = pure `buildAdvanceLadder` (knockout periods × playoff entries → enters/−cut/survives rows, ACTUAL eliminated counts for cut rounds, next-up pointer, champion) + an SSR DRY-RUN of the next uncut round through the SAME orchestrator + verbatim store — the panel's refusal banner and the write route's refusal are the same guard (the §28.5 no-disagreement rule, applied to a whole orchestrator). The alive field renders leaderboard-style with a dashed cut line before the cut zone; a residual boundary tie renders a chip picker (exactly `cutsRemaining` picks) whose completion triggers a breakTie DRY-RUN so the SERVER names the full eliminated set before any confirm; apply is type-to-confirm `CUT` with copy naming the eliminated managers + counts (+ champion) and stating irreversibility. `packages/scoring` + `packages/recompute` byte-untouched; `apps/worker` = re-points only. See PROJECT.md / DECISIONS.md → 2026-07-02 (Thread 5).

**Cut + release extension (`feat/commish-advance-release`, MERGED main `c37001b`, 2026-07-03).** The Thread-5 scope-out ("no roster/FAAB/scoring writes") is REMOVED for the just-cut managers only: `applyRoundCut` now flips the cut AND releases those managers' entire active rosters to the wire in the SAME `$transaction`, on BOTH surfaces. The web adapter (`commishAdvanceStore`) mirrors the relocated adapter exactly and adds the release + the one `round_advance` audit row (now with a `released` map + `releasedCount` in `target_ref`, no migration). The release reuses `@app/faab/prisma`'s `releaseEliminatedRosters`, which threads the outer `tx` into the shared `releaseRoster` body via the **optional-tx enlistment pattern** (see §21) — the pure `FaabReleaseStore` port is unchanged, so `commish:trim`/manager release stay behavior-identical. Locked slots release under the `app.commish_override` GUC scoped to the cut round (`periodId: round.id`); the idempotency claim fronts the release; the dry-run plan carries `releasePreview`. Gate: `-w typecheck`/lint/format/`vitest` **3073** + web build; gated-PG commishAdvance **7/7** · advanceStore **4/4** · faab release **6/6**. See DECISIONS.md → "Cut + release — ONE atomic action" + [[commish-5-advance]].

**Downstream of the cut — the first-open ceremony (`feat/cut-ceremony-first-open`, 2026-07-04).** The `round_advance` write here is what a round's `live→past` flip ultimately reflects; the client ceremonies that reveal it (Theater blade + The Cut takeover) now play on the FIRST device open after an unseen cut, not only for a live watcher, via the shared per-device `seenCeremony` latch. That is purely client presentation state — this apply path and the clockless `loadPlayoffs`/`buildPlayoffsView` read contract are byte-untouched by it. See §21 (Theater → first-open cut ceremony) + DECISIONS.md → 2026-07-04.

**Auto-fire (`feat/autofire-round-cut`, MERGED main `98bf720` + DEPLOYED, 2026-07-04) — a THIRD in-worker caller of `runRoundAdvance`.** The resident scheduler tick gains a LAST step (after status-advance, so a just-finished round is already `closed`; its OWN try/catch = isolation) that auto-fires the cut+release for a knockout round once it is SCORE-COMPLETE, behind `AUTOFIRE_CUTS_ENABLED` (default OFF ⇒ short-circuits before any IO). New module `apps/worker/src/autofire/`: the pure `selectAutoFireCut` picks the earliest uncut CLOSED round (past a secondary settle floor); the pure **`selectRoundDataComplete`** is the PRIMARY gate — every fixture completed + populated + fully-rated (`rating_player_match`) + swept (no dirty stat/rating inputs), and the period's `manager_period` `recompute_dirty` drained, REUSING `playerAppearedInMatch` (appearance not reinvented), fail-safe HOLD on any doubt (a never-rated appeared player holds for manual `commish:advance` — logged, no timer escalation). A DRY-RUN `runRoundAdvance` classifies; a DETERMINED cut applies via `runRoundAdvance({apply:true, allowIncomplete:true, SYSTEM actor})` against `createPrismaAutoFireAdvanceStore` — a store that MIRRORS the web `commishAdvanceStore.forAdvance` statement-for-statement (the `alive→eliminated` claim + champion flip + `releaseEliminatedRosters`) and adds a durable NULL-actor `auto_advance` `commish_audit` row via the shared **`@app/commish-core/advanceAudit`** builder (`buildAdvanceAuditRow`/`recordAdvanceAuditTx`, byte-identical to the web `round_advance` shape), all in the SAME `$transaction` (a forced audit failure rolls the cut+release back). A tie routes to `dispatchCommissionerAlert` (see the notify §), never a cut. `runRoundAdvance`/`advanceStore`/`resolveRoundCut`/`recordCommishAudit`/the web route are byte-untouched. Follow-up debt: the now-3-way apply-tx mirror + the advanceAudit dedup (DECISIONS.md → Auto-fire the playoff round cut). See DECISIONS.md → 2026-07-04 + [[autofire-round-cut]].

**Consolidation (`feat/commish-core-consolidation`, MERGED main `99c653c` + DEPLOYED, 2026-07-04) — the three cut-apply mirrors collapse onto ONE writer.** The follow-up debt above (a 3-way mirror of the cut `$transaction` + an un-deduped web audit builder) is closed. `advanceStore.ts` extracts the module-private `runRoundCutInTx(tx, input, recordAudit?)` — the SINGLE claim → champion → `releaseEliminatedRosters` → optional-audit body; `createPrismaPlayoffAdvanceStore().applyRoundCut` self-opens its `$transaction` and takes an optional tx-bound `recordAudit` hook. The web `commishAdvanceStore.forAdvance` and the worker `autofire/advanceStore` DELETE their hand-mirror write bodies and CALL the canonical writer with a per-surface hook: web = `round_advance`/real actor (still via the injected `buildAudit` → pinned `recordCommishAudit` seam + `auditId()`), auto-fire = `auto_advance`/null actor (via the shared `recordAdvanceAuditTx`), CLI = no hook (stdout only). `handleAdvance.buildAdvanceAudit` is deduped onto the shared `buildAdvanceAuditRow` — ONE builder feeds both durable callers. The hook rides the CONCRETE `PrismaPlayoffAdvanceStore` only; the pure port + memory double stay byte-untouched (root export `@app/db`-free). An optional-`tx` enlistment param (mirroring `releaseRoster`, §21) was added then DROPPED in review as dead (no caller passed it) → `applyRoundCut` always self-opens; `runReleaseInTx`/`releaseRoster`'s own tx param (used by `releaseEliminatedRosters`) is untouched. Fences empty: `advance.ts` orchestrator, the release primitives' internals, FAAB engine, `resolveRoundCut`/cut math, `advanceAudit.ts`, `recordCommishAudit.ts`; no schema/migration/RLS/Realtime, no new `action_type`; ZERO test files changed. Gate green (typecheck/lint/format/**3139**/web build) with **all four gated-PG pins (advanceStore 4 · autofire 5 · commishAdvance 7 · faab release 6) UNMODIFIED** — the drift pins prove no behavior changed. See DECISIONS.md → 2026-07-04 (Commish-core consolidation).

### §28.7 — Thread 6: console closeout (`feat/commish-closeout`, source-only, LAST thread)

**Draft-setup tab removed.** The 5th `CommishConsole.tsx` tab (an inert placeholder for a draft that already ran and completed pre-tournament) is deleted along with its dead `adm-*` CSS; console goes 5 tabs → 4 (Audit log, Stat corrections, Roster & lineup, Game operations, Playoff cuts). `/draft` (the historical draft room) is untouched.

**The `DECISIONS.md:3397` nav seam is closed.** Thread 1 shipped `commish` as a `NavId` gated behind an optional `AppShell`/`MoreSheet` `isCommissioner` prop, but only the hub page and `/commish`'s own layout threaded it — every other screen showed no commissioner entry point. This thread closes that: `getSessionManager()` is wrapped in React `cache()` (request-scoped memoization — one Supabase+Prisma round-trip shared by every caller in that request, not per-layout), and a new pure `getViewerIsCommissioner()` (`apps/web/lib/auth/manager.ts`) wraps it. All 10 feature layouts (`dashboard`, `draft`, `lineup`, `vsfield`, `waivers`, `pool`, `standings`, `playoffs`, `games/[matchId]`, `settings`) are `async`, call it, and pass `isCommissioner` into `<AppShell>`. `deriveIsCommissioner` — the pure predicate delegating to `resolveCommissioner` (`@app/shared`, the same one `/commish`'s own gate uses) — lives in its own dependency-free module (`apps/web/lib/auth/deriveIsCommissioner.ts`) so it unit-tests without pulling in the `server-only` guard that `manager.ts` carries.

**Blocked-banner copy mapping (display layer only).** `apps/web/src/commish/advanceRefusalCopy.ts`'s `mapAdvanceRefusal()` pattern-matches the Thread-5 orchestrator's one CLI-flavored refusal string (`"...wait for the result freeze, or pass --allow-incomplete"` — dead on the web surface since `handleAdvance.ts` hardcodes `allowIncomplete: false`) and rewrites it to `"...freeze the round in Game operations first."`; every other refusal passes through verbatim. Applied at both `handleAdvance.ts`'s `toHttp()` (the POST response) and `loadCommish.ts`'s SSR dry-run preview, so the banner never disagrees with itself pre- vs post-apply. `packages/commish-core/src/advance.ts` is byte-verbatim.
...
## §30 — T15-2 shell stacking: the z-scale, the sheet-chrome kit, and the bottom-nav tap fix (`feat/shell-stacking`, MERGED `--ff-only` 2026-07-04)

**The z-scale (ds.css `:root`, all five copies in lockstep).** ONE documented stacking ladder replaces the per-surface folklore: `--z-nav:100` (the fixed mobile bottom bar; 101/102 reserved for the shell's own More backdrop/sheet) `< --z-overlay:120` (every route modal/sheet scrim, floating toasts, dropdowns) `< --z-overlay-stack:130` (an overlay opened on top of an open overlay — the FA card from inside the bid composer) `< --z-takeover:200` (score sheet / forfeit confirm). Consumers moved onto tokens: `.pl-modal-overlay` (50→overlay), `.pc-scrim` (80→overlay; the §29 `/players` route-scoped 120 override retired), `.dr-toasts` (80→overlay + phone bottom-offset clear of the nav band), `.wv-scrim` (90→overlay), `.wv-app .pc-scrim` (95→overlay-stack), `.adm-viewas-menu` (40→overlay), `.sl-forfeit-overlay` (200→takeover token, same computed value). The Cut's `ko-sheetwrap`(110)/`koc`(120) already conform and stay LITERAL — T15-CUT fenced byte-untouched (`verify-the-cut.mjs` 43/43 UNMODIFIED). The nav is additionally **pointer-inert under any open route scrim** via `body:has(...)` (More backdrop/sheet excluded — they render INSIDE `.sh-btmnav`; ko surfaces excluded — fenced). **Cascade trap pinned:** the four per-route ds.css copies (draft/lineup/vsfield/_landing) load AFTER the canonical file — a change only in `app/styles/ds.css` gets re-overridden, so all five change identically (`appShell.test.ts` byte-identity).

**Bottom bar (F-P0-A1 + 6-slot spacing).** `.sh-btnav-item` = `flex:1 1 0; min-width:0` (EQUAL slots — flexbox's `min-width:auto` had let nowrap labels squeeze "Players"/"More" under 44px and overflow the bar below ~340px effective width) + `min-height:44px` + label-span ellipsis guard + `:active` press feedback + `touch-action:manipulation` (an MPA `<a>` gives no other pre-navigation signal; tap-highlight was suppressed). Bar height stays ≤58px — the `.sh-content` clearance constant other harnesses assert. Full diagnosis (incl. the ruled-out hypotheses and the two runtime-only iOS contributors) lives in `audit/T15-2_SHELL_STACKING_NOTES.md`; the REPORTED-not-fixed residual is the MPA navigation latency itself (prefetch/progress = routing behavior, own clearance).

**The sheet-chrome kit.** `apps/web/components/useSheetChrome.ts` — stacked-count body-scroll lock + Escape (latest-handler ref) + focus trap + opener focus-restore; consumed by MoreSheet, BidComposer, FaPlayerCardSheet, PlayerScoreSheet, ForfeitConfirmSheet, pool ManagerPicksModal. CSS pair: `overscroll-behavior:contain` on MODAL scrollers ONLY (the in-flow FA panel shares `.wv-comp-*` classes and must keep page-scroll chaining — modal instances are scoped `.wv-scrim .wv-comp-*`); all modal caps dvh (88/90/85/80); scrims carry `env(safe-area-inset-*)`. Sheets restructured to FIXED chrome (✕/header/tab strip) over an internal scroller (`.pc-body`, new `.sl-sm-body`) so close/tabs never scroll away (F-P2-PSC1); close targets 44px. MoreSheet gains the reference chrome (`.sh-sheet-grab/head/title/x`) + `aria-modal` (F-P2-A4/F-P3-A1). Safe-area-x on bar/sheet/topbar/content (F-P3-A2); commish console bottom clears the home indicator (F-P3-G3 resolved static — the console mounts inside AppShell, whose `.sh-content` already reserves the <640 nav band).

**Step-27 (in-flow, not z).** The instant-pickup `.wv-fa` section's action column orders ABOVE the pool list at ≤768px (`order:-1`); the sealed-bid MODAL instead scrolls internally (`.wv-comp-body` overflow at ≤768px). Harness-found production bug fixed: `.wv-composer` `width:780px + max-width:100%` rendered 780px wide at phone widths inside the grid-centered scrim (percentage-vs-auto-track circularity) → `width:min(780px,100%)` (the `.pl-modal` pattern).

**Proof.** `verify-shell-stacking.mjs` (pw.local replica harness, real CSS) 33/33 at 360/390/430+desktop: overlays above nav with clickable ✕/action rows, nav tap-proof under scrims (REAL dispatched click must not navigate), SaveBar above the band (F-P1-C1: `bottom: calc(58px + env(safe-area-inset-bottom) + 10px)` <640px), step-27 reachable, REAL tap-through on all 5 tabs + a More item, equal ≥44px slots, no h-overflow. Vitest side: `shellStacking.contract.test.ts` (byte pins) + `moreSheetChrome.dom` + `useSheetChrome.dom`. See DECISIONS.md → 2026-07-04 (T15-2) + PROJECT.md session log.
