...
**Pitch lane-wrapping (`feat/pitch-lane-wrapping`, 2026-06-22, merge HELD — presentation/layout only).** With the reconciled XI guaranteed to be exactly 11, the only remaining pitch defect was layout: each position band rendered as a SINGLE flex line (a vertical column on desktop, a horizontal row on mobile) inside the fixed-height pitch (`.gd-pitch` 516px desktop / 600px mobile, `overflow:hidden`), so a deep band (5–6 mids/defenders — confirmed Uruguay–Cape Verde) overflowed and clipped off the bottom (desktop) or the edge (mobile). **The fix is purely how `PitchHalf` lays out `side.pitch` — no loader/builder/data-contract/engine change; `buildGameDetail.ts`/`types.ts`/`loadGameDetail.ts` are byte-untouched.**

- **Pure grouping helper `apps/web/src/games/pitchRows.ts` (`pitchRows.test.ts`).** `pitchRows<T>(players, narrow?)` splits a band of N into balanced formation lines so none overflows: 5 → `[2,3]`, 6 → `[3,3]`, 7 → `[3,4]`, 8 → `[4,4]` (`MAX_PER_LINE = 4`, `ceil(N/MAX)` lines) — identical on both axes. It returns the lines in **BACK→FRONT** order with the **fuller line biased toward the front (toward midfield)** — a 5-mid band reads "a line of 3 with a line of 2 behind". The helper is **orientation-aware in exactly one case, the flat back-4**: on the WIDE (desktop) axis ≤4 → one line; on the NARROW (mobile) axis ≤3 → one line and **4 → balanced `[2,2]`** (back 2 | front 2), so a back-4 wraps to a deterministic 2+2 on a phone rather than a single 4-wide row. `narrow` is the same desktop/mobile signal the half uses to flip flex-direction (no new breakpoint). Generic + pure (no `PlayerLine` dependency, no mutation); presentation logic only — it does NOT live in the builder.
- **Render (`GameDetailClient.PitchHalf` → `LaneColumn`).** Each lane `.gd-pcol` holds one `.gd-pline` per formation line (the old single-column case is just one `.gd-pline` → visually identical). A wrapped (multi-line) lane gets `.gd-pcol.is-wide` for extra depth so each sub-line stays as roomy as a normal lane. **Back-4 dual-render (the one axis-divergent band):** SSR can't know the viewport, so `PitchHalf` computes both groupings and, when they differ (only the back-4: wide `[4]` vs narrow `[2,2]`), renders two variants (`.gd-pcol.is-wide-only` / `.is-narrow-only`); the `max-width:720px` breakpoint that flips the half's flex-direction also toggles their `display`. All other bands match across axes → rendered once. The 2+2 is thus **helper-driven, not flex-overflow-driven** — `.gd-pline`'s `flex-wrap` stays only as a last-resort clamp so an unusually wide sub-line (e.g. a 7→`[3,4]` front line on a tiny phone) can't clip.
- **The orientation invariant: `.gd-pcol` flex-direction MATCHES its parent `.gd-phalf`.** The team's GK→FWD axis and a band's back→front axis point the same way, so the back→front line order renders correctly for home AND away, desktop AND mobile, with no per-side/per-viewport branching in the helper. Desktop: half is `row`/`row-reverse` → `.gd-pcol` matches, `.gd-pline` is a vertical column of tokens (spread across the 516px height). Mobile (vertical flip, away-top/home-bottom): half is `column`/`column-reverse` → `.gd-pcol` matches, `.gd-pline` is a horizontal token row. **Mobile `.gd-pitch`/`.gd-pitch-grid` height becomes a `min-height` floor (not a cap) and halves/lanes size to content (`flex-shrink:0; flex-basis:auto`)** so a wrapped band grows the pitch downward instead of clipping — the page scrolls; desktop stays the fixed 516px (two sub-columns of ≤4 always fit the height).
- **Subbed-off ↓ badge on the pitch token (Step 2B).** Pre-fix the `▾min′`/red glyphs were list-only (`RowGlyphs` → `LineupRow`); the pitch `KitToken` showed none. It now renders a `.gd-tok-status` pill overlaid on the jersey hem (reusing the existing `.gd-rev is-off`/`.gd-rev is-red` glyph classes) for a subbed-off (`wentOffMinute`) or red-carded (`redCard`) starter — `▾{minute}′` visible at any token size, the replacement name (`subbedOffForName`) in the `title` (kept off the small token face; it shows in full in the Starting XI list below). **Presentation-only: every field was already on `PlayerLine` from the 8385c93 reconciliation; the only VM gap is the red-card MINUTE (not carried — only the boolean), so a red badge shows the marker, no minute.** A come-on sub never gets a pitch token (he is a `sub`, not in `side.pitch`), so only the ↓ badge is meaningful on the pitch.

See PROJECT.md → 2026-06-22 (pitch lane-wrapping).
