...
### Audit
One `commish-override` audit line per applied cut (per eliminated manager, `action: "eliminated"`) + one for the champion flip (`action: "champion"`), each carrying the `round`, `reason`, and `tieAdjudicated` flag — plus, since the cut+release extension, the `released` player ids shed to the wire per manager (mirrors `commish:trim`'s `released`). Reuses `formatAudit` (structured stdout, no table). The WEB surface records the same in the `round_advance` `commish_audit` row's `target_ref` JSONB (`released` map + `releasedCount` — no migration, the column is JSONB).

### Cut + release — ONE atomic action (`feat/commish-advance-release`, MERGED main `c37001b`, 2026-07-03)
The apply is now ONE atomic action that BOTH flips the round's cut AND releases the just-cut managers' ENTIRE active roster to the free-agent wire, on BOTH surfaces (CLI `commish:advance` + `POST /api/commish/advance`), inside the SAME `$transaction` as the cut. This removes the earlier scope-out — **for the just-cut managers only**; the champion + previously-eliminated managers are never released.

- **Reuse, not duplication.** `releaseRoster` (`@app/faab/prisma`) gains an optional `tx` param — when present it ENLISTS in the caller's outer transaction instead of opening its own; the body is factored to a shared `runReleaseInTx`. The `tx` param rides the CONCRETE adapter ONLY, not the pure `FaabReleaseStore` port (`purity.test.ts` forbids `@app/db` in the port), so the manager route + `commish:trim` stay behavior-identical (self-opened tx, unchanged — pinned by faab `release.integration` 6/6). A new exported `releaseEliminatedRosters(tx, {leagueId, managerIds, roundPeriodId, at})` reuses `runReleaseInTx` to drop each cut manager's whole active roster + release slots, returning the released ids per manager.
- **Atomicity.** `applyRoundCut` (the relocated commish-core adapter AND the web adapter that mirrors it) runs `alive→eliminated` claim → champion flip → `releaseEliminatedRosters` → (web) the ONE `round_advance` audit insert in ONE `$transaction`. A failure (e.g. an FK-violating audit row) rolls back the cut AND the release together — never a cut with un-released rosters, never a release without the cut.
- **Idempotency gates the release.** The conditional `alive→eliminated` claim (0 rows ⇒ already cut) fronts the release: a re-run is `already-cut`, re-drops nothing, mints no audit row.
- **Locked-slot scope (by design).** The release passes `periodId: ctx.round.id` + `allowLocked: true` per manager, mirroring `commish:trim`. This releases the cut managers' LOCKED `lineup_slot` rows for the round they just played under the `app.commish_override` GUC (the lock-on-play DELETE trigger exempts it), scoped tightly to the cut round; survivors are never enumerated. Their `score_manager_period` for that round is already FROZEN (the cut precondition) and recompute-on-frozen is gated off, so stored scores/standings are UNAFFECTED — but this DELIBERATELY clears the eliminated managers' final-round lineup snapshot (Chat-cleared; the lever to preserve it instead is `periodId: null`, which keeps closed-round slots).
- **Dry-run preview.** A new `PlayoffAdvanceStore.loadActiveRosters` populates `AdvancePlan.releasePreview` (per-cut-manager players) for a `determined` cut — a pure read; surfaced in the CLI report + the `/commish` Playoff-cuts tab + the type-to-confirm copy, so the full blast radius is visible before apply.
- **Scope fences byte-untouched** (empty diffs): `selectGuillotineCuts`, `resolveRoundCut`, faab `faEligibility`/`resolve`, scoring, recompute. Gate green: `-w typecheck`/lint/format/`vitest` **3073** + web build; gated-PG **commishAdvance 7/7 · worker advanceStore 4/4 · faab release 6/6**. See ARCHITECTURE.md §21/§28.6 + [[commish-5-advance]].

### Auto-fire the playoff round cut (`feat/autofire-round-cut`, MERGED main `98bf720` + DEPLOYED, 2026-07-04)
The resident worker tick now AUTO-fires the combined cut+release for a knockout round once it is score-complete — behind the `AUTOFIRE_CUTS_ENABLED` kill-switch (default OFF ⇒ a byte-identical no-op) and idempotent. It INVOKES the untouched `runRoundAdvance` (the SAME entry `commish:advance` uses) with a SYSTEM commissioner actor + `apply:true`; it re-implements NO cut/release/resolution math. A boundary tie is NEVER auto-cut. New module `apps/worker/src/autofire/`.

- **The fire gate is DATA-COMPLETENESS, not a clock.** A "settle N minutes after the last kickoff" gate is a near-no-op (no completed-at column exists — the only FT proxy is `max(kickoffAt)`, and the round's period is already `closed` = every fixture completed by the time we look) AND unsafe — it could fire on a final match whose RATINGS have not landed, and ratings are the LAST-arriving scoring input while the round score IS the cut criterion. So the pure `selectRoundDataComplete` (`autofire/completeness.ts`) requires, for EVERY fixture in the round's period: (1) `status = completed`; (2) ≥1 APPEARED player (a real completed match always has some — zero ⇒ feed not ingested ⇒ hold); (3) every appeared player (the REUSED `playerAppearedInMatch` participant gate — appearance NOT reinvented) has a rating ingested (`rating_player_match.rating != null`; provisional fine, ABSENT not); (4) no dirty `stat_player_match`/`rating_player_match` inputs — AND the period has no pending `recompute_dirty` (scope `manager_period`) so its `score_manager_period` is drained. Fail-safe: any unconfirmable condition ⇒ HOLD (never fire). `AUTOFIRE_SETTLE_MIN` (default 5) is retained only as a harmless SECONDARY floor.
- **The never-rated-appeared-player HOLD rule.** If an appeared player never gets a rating, condition (3) never clears ⇒ the round NEVER auto-fires — it holds for manual `commish:advance --allow-incomplete` indefinitely rather than firing on incomplete data. The hold is VISIBLE (a per-tick `log.warn("autofire.cut.incomplete", …)` naming the unrated players) — NOT silent — and there is deliberately NO timer escalation (that would reintroduce the clock this gate removes). Fail-safe beats liveness for an unattended irreversible cut; manual advance is always the fallback.
- **This is what justifies `allowIncomplete:true`.** At fire time the round is closed + data-complete but NOT yet frozen (freeze waits `result_freeze_hours`). Auto-fire passes `allowIncomplete:true` to cross the frozen precondition — legitimately, because the data gate guarantees the scores are PRESENT (provisional), never ABSENT. The web advance route keeps `allowIncomplete` PINNED false (unchanged).
- **Durable null-actor `auto_advance` audit in the cut `$transaction`.** The apply writes a `commish_audit` row INSIDE the same `$transaction` as the cut+release (atomic — a forced audit failure rolls BOTH back, gated-PG-proven), replacing the CLI/worker stdout-only audit. `actor_user_id = NULL` (the nullable-by-design system row); a new free-TEXT `action_type` `"auto_advance"` (added to `COMMISH_ACTION_TYPES` — NO migration) legibly separates auto from operator `round_advance`; the `target_ref` (eliminated/champion/released/releasedCount) is byte-identical to the web path.
- **Tie-hold alert.** A `needsCommissioner`/`invalid-tiebreak` resolution dispatches a ledgered commissioner push via `dispatchCommissionerAlert` — an ADDITIVE, un-gated sibling of `dispatchToManager` (no preference gate; a governance nudge isn't opt-out), free-TEXT kind `cut_needs_review`, at-most-once per round via the `notification_sent` UNIQUE ledger. `dispatchToManager` + the closed `NotificationKind` union are byte-identical (the ledger `kind` is free TEXT; a widened `LedgerKind` superset types `claimLedger` only).
- **Follow-up debt (RECORDED → ✅ RESOLVED 2026-07-04, `feat/commish-core-consolidation` main `99c653c`).** (a) There were THREE mirrors of the cut-apply `$transaction` — the relocated commish-core adapter, the web `commishAdvanceStore`, and the worker `autofire/advanceStore` — pinned against each other by the gated-PG suites but a dedup candidate. (b) The shared `@app/commish-core/advanceAudit` builder is byte-identical to the web `buildAdvanceAudit`+`recordCommishAudit`, but the web copies were left UNTOUCHED (fence); de-duping the web onto the shared builder is a follow-up. **Both resolved:** the three mirrors collapse onto ONE canonical `runRoundCutInTx` writer (self-opens; optional per-surface `recordAudit` hook) and the web builder is deduped onto `buildAdvanceAuditRow` — see "Commish-core consolidation" below.
- **Scope fences byte-untouched** (empty diffs): `resolveRoundCut`/`selectGuillotineCuts`/`runRoundAdvance` internals/`advanceStore`/`releaseEliminatedRosters`/FAAB engine/the web advance route/`recordCommishAudit`/sweep+recompute engine/freeze+status selectors/`schema.prisma`. No schema/migration/RLS/Realtime. Gate green: `-w typecheck`/lint/format/`vitest` **3139** + web build; gated-PG advance **4/4** + autofire **5/5** (determined⇒cut+release+durable audit over an unfrozen-but-closed round; 2nd tick no-op no dup; data-incomplete⇒hold no cut no audit; tie⇒one `cut_needs_review` row no dup; forced audit failure rolls cut+release back). **`AUTOFIRE_CUTS_ENABLED` is the operator on-switch** (Render dashboard, default off). See ARCHITECTURE.md §28.6 + the notify § + [[autofire-round-cut]].

### Commish-core consolidation — ONE canonical cut+release+audit writer (`feat/commish-core-consolidation`, MERGED main `99c653c` + DEPLOYED, 2026-07-04)
The three hand-mirrored copies of the cut-apply `$transaction` (the follow-up debt above) are collapsed onto ONE canonical writer — a pure STRUCTURAL refactor, behavior-preserving on every surface (CLI, web, auto-fire).
- **One writer.** `packages/commish-core/advanceStore.ts` extracts the module-private `runRoundCutInTx(tx, input, recordAudit?)` — the single source of the `alive→eliminated` claim → champion flip → `releaseEliminatedRosters` → (if a hook is given) the durable audit insert. `createPrismaPlayoffAdvanceStore().applyRoundCut(input, opts?)` self-opens its `$transaction` and takes an optional tx-bound `recordAudit` hook. Both mirror WRITE bodies are DELETED: the web `commishAdvanceStore.forAdvance` and the worker `autofire/advanceStore` now CALL the canonical writer and pass their own hook, re-implementing no claim/champion/release statement.
- **Per-surface audit config.** Web → `round_advance`, real actor, still built by the injected `buildAudit` and inserted through the pinned `recordCommishAudit` seam (so the atomicity gated-PG probe — a custom `buildAudit` returning an FK-violating row — is byte-unchanged) + `auditId()` capture. Auto-fire → `auto_advance`, null actor, via the shared `recordAdvanceAuditTx`. CLI → NO hook (stdout `formatAudit` only, no `commish_audit` row — exactly as before).
- **Audit builder dedup.** `handleAdvance.buildAdvanceAudit` no longer re-implements the summary/detail/delta/`target_ref` strings — it delegates to the shared `buildAdvanceAuditRow` and re-shapes into `RecordCommishAuditInput`. ONE builder now owns the round-cut audit shape for BOTH durable callers, byte-identical (pinned by `advanceAudit.test.ts` + `handleAdvance.test.ts`).
- **Purity + the dropped tx param.** The `recordAudit` hook rides the CONCRETE `PrismaPlayoffAdvanceStore` only; the pure `PlayoffAdvanceStore` port and the memory double are byte-untouched, so the `@app/db`-free root export holds. The thread's first cut also gave `applyRoundCut` an optional-`tx` enlistment param (mirroring `releaseRoster`, §21), but the pre-merge review DROPPED it as dead (no caller passed it — exactly the `releaseRoster` precedent's own unused-tx state) → `applyRoundCut` ALWAYS self-opens. `runReleaseInTx`/`releaseRoster`'s own tx param is untouched (it IS used by `releaseEliminatedRosters`).
- **Scope fences byte-untouched** (empty diffs): `advance.ts` (`runRoundAdvance` orchestrator), `releaseRoster`/`runReleaseInTx`/`releaseEliminatedRosters` internals, FAAB engine (`resolve`), `resolveRoundCut`/`selectGuillotineCuts`/cut math, `advanceAudit.ts`, `recordCommishAudit.ts`. No schema/migration/RLS/Realtime, no new `action_type`; ZERO test files changed. Gate green: `-w typecheck`/lint/format/`vitest` **3139** + web build; **all four gated-PG drift pins pass with UNMODIFIED expectations** — advanceStore **4/4** · autofire **5/5** · commishAdvance **7/7** · faab release **6/6** (the whole point: no assertion moved ⇒ no behavior changed). Both services deployed green. See ARCHITECTURE.md §28.6 + [[commish-5-advance]] + [[autofire-round-cut]].

### Scope (what this phase does NOT do)
Beyond the cut + release above, the apply does NOT recompute scores (it READS `score_manager_period`) or flip `league.status`. **⚠️ Seam (open — routing not yet decided):** the `league.status` `playoff → complete` flip is still deferred. The next thread's design step must first resolve: (a) does the UI derive "tournament complete" from the champion `playoff_entry` row existing (preferred — single source of truth, no write, no drift), or does it keep `league.status` as the phase source? If (b) is chosen, the `playoff → complete` flip is a WRITE belonging in the commish/application layer — folded into the champion `$transaction` in `applyRoundCut`, or a dedicated commish step — **not** in the read-only `loadPlayoffs` loader / UI thread. Either way: functionally harmless while deferred (a champion league has 0 `alive` entries, so the FAAB gate and standings logic already exclude them).
...
## 2026-07-04 — Cut ceremony fires on FIRST OPEN per device — the shared `seenCeremony` latch (`feat/cut-ceremony-first-open`, review-class, **MERGED `--ff-only` + DEPLOYED** main `463089a`)

**Context.** Both knockout ceremonies — the `/vsfield` "The Cut" `KOCeremony` takeover (§27) and the `/playoffs` "Theater" blade choreography (2026-06-30, Decision 2) — fired ONLY for a client actively watching a round flip `live→past` between refetches. A manager who opened the tab AFTER the cut landed saw the settled aftermath and MISSED the reveal. This adds a per-device "seen cut ceremonies" latch so the ceremony plays the FIRST time a device opens either surface after an unseen cut — exactly once — then settles on every later open.

**Decision (a) — one SHARED client utility; the clockless loader UNTOUCHED.** `apps/web/src/playoffs/seenCeremony.ts` is the single source both surfaces import: a per-device localStorage seen-set keyed `xi:seenCut:<leagueId>:<roundLabel>` (`seenCutKey`), storage-safe `hasSeenCut`/`recordCutSeen`, and the pure `decideCeremonyLatch`. No loader/view-model/engine touch — `loadPlayoffs`/`loadVsField`/`buildKnockoutContext`/`buildPlayoffsView`/`guillotine`/`playoffRound` are git-verified BYTE-UNTOUCHED (the clockless STOP seam holds; this is purely client presentation state). `leagueId` (absent from the clockless `PlayoffsView`) is threaded to `PlayoffsClient` via a PARALLEL indexed `manager.findUnique` in `page.tsx` — NOT by extending the fenced loader; `/vsfield` already carries `initialView.leagueId`.

**Decision (b) — mount-effect ⟂ live-latch: one fire per round per device.** Each surface KEEPS its existing between-refetch `live→past` latch (a live watcher still sees it real-time) and GAINS a CLIENT-ONLY mount-once effect (`useEffect`, never SSR/render → no hydration mismatch) that diffs the PAST cut rounds the view-model already exposes against the seen-set. The two are DISJOINT: on mount `prev=null` ⇒ the live latch no-ops, so the mount effect owns the first-open fire; a later genuinely-new flip is owned by the live latch. BOTH now `recordCutSeen` when they fire, into the SAME key — so a fire on either path OR the other surface suppresses every later replay (cross-surface no-replay proven by shared-key RTL + render tests).

**Decision (c) — fire rule = most-recent-past-cut IFF unseen (not "most-recent unseen").** `decideCeremonyLatch` fires ONLY the most-recent past cut and ONLY when it is unseen; older unseen cuts are recorded but NEVER fired (no backlog chain-replay). This deviates from the literal prompt ("most-recent UNSEEN") only in the unreachable-in-normal-flow "latest seen, older unseen" case — chosen because (1) it honors the prompt's own "older cuts never replay" and (2) `/vsfield`'s `KOCeremony` can render ONLY `ko.settled` (the most-recent settled round), so cross-surface coherence REQUIRES the fired round to be that one. `record` = ALL past labels, so a fresh device seeds its whole past in one mount.

**Decision (d) — fire-latest-once default; reduced-motion records-but-static; fail-toward-seen.** DEFAULT = fire the single latest past cut on a cold device (`SEED_SILENT_DEFAULT = false`; the seed-silent variant — record-all, animate nothing — is one const flip, cold-start-only). Reduced-motion keeps the existing gating: `/vsfield` still OPENS the ceremony (`KOCeremony` shows the static `is-aftermath`), `/playoffs` skips the swing — BOTH still record. A throwing/blocked localStorage FAILS TOWARD "seen" (no full-screen-takeover replay-spam on every load; the in-session live latch remains the fallback). The champion/complete endgame is a PERSISTENT resting state — explicitly OUT of the one-shot latch.

**Invariants + gate.** Fences byte-untouched (loader/view-model/engine + `ds.css`/`knockout.css`/`playoffs.css`/`KnockoutUI.tsx`/`components.tsx`); `packages/` diff EMPTY; no schema/migration/RLS/Realtime. Full DoD gate green (typecheck/lint/format/**3094 passed / 96 skipped**/`@app/web` build) + `verify-the-cut.mjs` **43/43** + `verify-playoffs-hero.mjs` all-green (both extended with a real-browser localStorage-gated latch proof: first-open-fires+writes-key / second-open-settles / cross-surface-no-replay / reduced-motion, desktop+mobile) + `ceremonyLatch.dom.test.tsx` (7, REAL clients mounted) + `seenCeremony.test.ts` (14, pure decision + storage). Review-class → cleared by Chat + **MERGED `--ff-only`** (`e454c84..463089a`); `wc-fantasy-web` deployed green (both `/vsfield` + `/playoffs` are web routes; worker untouched). See PROJECT.md → 2026-07-04 + ARCHITECTURE §21 (Theater) + BACKLOG → Thread B. Related: [[t15-cut-the-cut-layer]], [[playoffs-chocoyo-reskin]].

## 2026-07-04 — PLAYERS-1: the `/players` full-tournament browser — acquisition single-sourced on /waivers, zero writes on /players (`feat/players-browser`, contained read-surface, **MERGED `--ff-only` + DEPLOYED**)

**Context.** A new READ-ONLY route to browse every tournament player (rostered + free), built to the committed design reference (`design/design_reference/players/`, `c0c7fce`). The design's eight artboards imply richer per-row data than the strict data model carries; the decisions below fence what ships.

**Decision (a) — DROP the row statline; rich stats live only in the card (G1, Sergio-ruled).** Six artboards center on a per-row stat table (`Pld·Min·G·A·Sh·KP·CS·Tkl·YC` + stat-header tooltips) fed by a `statsFor()` mock. But (1) the strict data model is name/position/nation/nationAlive/seasonPoints/ownership — nothing more; and (2) CS and YC have NO clean per-row source (`stat_player_match` has no clean-sheet/yellow-card column; CS is engine-derived, YC un-promoted). Ruling: rows use the reference's own simpler `.pl-row` vocabulary (Pos·Kit·Player/nation·Owner·Pts·Action, frame 6); the full statline lives ONLY in the shared card's Stats tab (the real `GET /api/player-tournament-stats`). The mobile pinned-column scroller, swipe-hint, stat-header tooltips, and the desktop 3-column tab-less card variant are DROPPED (design deviations, flagged in-plan). No second stats surface, no partial-column compromise.

**Decision (b) — acquisition is SINGLE-SOURCED on /waivers; /players has NO write path.** /players ships zero mutations (no bid/add/drop/star/watchlist). A free-agent row's only affordance is a trailing `<a href="/waivers?bid=<id>">` that NAVIGATES; `/waivers` reads the param server-side and `WaiversClient` PRESELECTS via the existing `setSelected` path (pure `resolveBidDeepLink`, mount-once) iff the window is open AND the player is still claimable — else a silent no-op. The bid submit path, FAAB engine, and composer/panel internals are byte-untouched (preselection only). Rationale: one acquisition surface = one place for the sealed-bid/FA/void rules to live; the browser stays a pure read. `packages/faab` + the bid routes are an untouched STOP seam.

**Decision (c) — the shared card is consumed CROSS-MODULE, not moved (R1).** `/players` imports `FaPlayerCardSheet` from `src/waivers/` directly. A physical move would break `faPlayerCardContract.test.ts`, which fs-pins the file's path to prove /waivers is unchanged — so the card gained two OPTIONAL props instead, both defaulting to today's /waivers behaviour with the LITERAL current strings (`onToggleStar?` gates the star render; `footNote` default = the verbatim /waivers copy). The full waivers suite staying green (incl. `faPlayerCardContract` + `faPlayerCard`) IS the byte-equivalence proof. The waivers→players import is a known, intentional seam; its eventual clean home in a shared location is a BACKLOG debt item (the move will re-point the contract-test pin). /players imports EXACTLY that one thing from /waivers — its row atoms are local.

**Decision (d) — nationAlive derivation + the Active-teams toggle SHIP.** The reference's Active-teams toggle needed a clean `nationAlive` source; `fifa_team.eliminated` (the commissioner-set flag with 5 FAAB readers, already a canonical column) IS one (`nationAlive = !eliminated`; null team ⇒ alive). So the toggle ships (no invented derivation was needed — the plan-checkpoint escape hatch of "ship without it" was not exercised). Eliminated players are VISIBLE by default (dimmed + struck); the toggle collapses them.

**Decision (e) — `/players` is NOT a nav tab; `AppShell.active` widened to `NavId | null`.** T15-2 owns the nav (no new tab, per the design's own "no new tab" note — the reference's active "Players" tab is a flagged deviation). Entry is a dashboard "Player pool" tile (static, group + playoff phases, `loadDashboard` untouched) + a "Browse all players" link on /waivers. To wrap /players in the shell without lying about the active tab, `AppShell`'s `active` prop was widened `NavId → NavId | null` (TYPE-ONLY + the single guard; every downstream consumer already accepted null). **T15-2 rebases onto this one-line widening.**

**Invariants + gate.** BYTE-UNTOUCHED: `packages/faab`, bid routes, `BidComposer`/`FreeAgentPanel` submit internals, scoring/recompute/ingest/lineup/auth; NO migration/schema/RLS/Realtime; `design/design_reference/` frozen. Mobile hard-rules held (≥44px, `dvh` via the shared sheet, card scrim z120 above the z100 bar, sticky ✕ + scroll containment). Tests: `playersLogic` (26), `loadPlayers.contract` (8, incl. R3 same-groupBy-as-loadWaivers), `playersClient` RTL (15), `waiversDeepLink` (14); `theCutSkin.test.ts` green (19). Full DoD gate green (typecheck/lint/format/pnpm test/`@app/web` build, `/players` = `ƒ`). Contained → **MERGED `--ff-only`** (feature commit the push tip → web deploy fired; docs commit `[skip render]` beneath). See PROJECT.md → 2026-07-04 (PLAYERS-1) + ARCHITECTURE §29 + BACKLOG → PLAYERS-1/PLAYERS-2. Related: [[faab-fa-live-unowned]].

## 2026-07-04 — PLAYERS-1 REMEDIATION: reverse G1 (real statline), fix dead entry points, honor the design density (`feat/players-remediation`, contained, **merge HELD for Chat clearance**)

Post-merge live verification of PLAYERS-1 found three gaps: the statline was dropped (G1), both entry points were effectively unreachable in-app, and the row density under-honored the design. This thread remediates all three. **Diagnosis-first** (the root causes differed from the initial hypotheses — recorded below). Merge is HELD (not autonomous) pending review of the screenshots + the YC decision.

**Decision (a) — G1 REVERSED; the statline is built to the REAL-DATA LIMIT.** The row is the design's `.mt-*` stat table again (a single horizontally-scrollable grid). Seven columns are REAL, from ONE `stat_player_match.groupBy` over appearances (`minutes_played > 0`): **Pld** (=`_count`), **Min**, **G**, **A**, **Sh** (the promoted `shots_on_target` — there is no raw-shots column, the `buildPlayerTournamentStats` convention), **KP**, **Tkl**. Display-only, from PROMOTED columns → `packages/scoring`/`recompute`/`ingest` are byte-untouched (grep-confirmed; the contract test pins no `prisma.eventMatch` read + no `@app/recompute` import in the loader).

**Decision (b) — YC premise was FALSE (extra), but YC is now REAL from `event_match`; CS is the ONE remaining gap.** The handoff said "YC = read from `stat_player_match.extra`". **DIAGNOSIS (live DB):** `extra` has NO yellow-card key (its keys are aerial_duels, big_chances_missed, crosses_total, xG/xA, fouls_committed, tackles). YC's real source is `event_match` (236 card events, class `yellow`/`red`). Initially rendered "—" pending the reviewer's call (it reads a scoring-INPUT table). **RESOLVED (PLAYERS-YC, Chat pre-cleared):** YC is now a REAL display-only count — `loadPlayers` reads `event_match` READ-ONLY and counts non-rescinded bookings via the SHARED `classifyCard` (`@app/recompute`, exported per T-CARD1 — the SAME predicate the game-detail Events timeline + the scoring engine use, so a second yellow is counted consistently). Cards are tournament-global (like `seasonPoints`). Importing an EXISTING pure export is NOT an engine change: `packages/recompute`/`scoring`/`ingest` are byte-identical (grep-confirmed empty `packages/` diff), no write, no migration, no score run. **CS stays "—"** — engine-derived, no per-row source: the ONE remaining gap, never fabricated. **SH confirmed = `shots_on_target`** — this MATCHES the shared card, whose "Shots"/"SH" also sources `shots_on_target` (`buildPlayerTournamentStats`); there is no total-shots column in `stat_player_match`, and the card doesn't read the `shot_match` table, so "matching the card" (the stated goal) means `shots_on_target` — kept for list↔card consistency.

**Decision (c) — KIT reuses the shared flag-kit `kitOf`, not the waivers text chip.** The KIT column renders the real flag-kit jersey via `kitOf` (`apps/web/src/kit` — the SAME primitive T13 lineup / T16 game-detail use; 30 WC nations + home nations, neutral fallback), NOT the /waivers text-abbrev `KitChip`. A small emoji `<Flag>` overlays so every nation carries a flag signal. `background-size: cover` is never set (the kitOf multi-layer contract). /players still imports EXACTLY ONE thing from /waivers (the shared card); the row atoms stay local.

**Decision (d) — mobile "← swipe stats" pinned-column table.** The `.mt-*` grid pins the Player cell left + the Pts cell right (`position: sticky`), stat columns scroll horizontally, `overscroll-behavior-x: contain` keeps the swipe off the page (T15 hard-rule), the bid trailer is a ≥44px tap target, and the page never overflows horizontally. It widens to a desktop wide-list at ≥760px (the swipe hint hides). Proven at 360/390/desktop by the Playwright render proof.

**Decision (e) — dead entry points fixed (root causes recorded).** **1a — waivers link:** DIAGNOSIS found it was NOT gated on claims-open (the handoff's hypothesis) — it was inside the `tab === "claims"` branch (invisible on the Batch-results tab). Fixed: moved to a persistent `.wv-browse-foot` OUTSIDE the tab ternary → renders in every phase incl. a locked window. **1b — dashboard tile:** DIAGNOSIS found the `Module` `cta` already renders as `<a href="/players">` and R16 resolves to `phase` ∈ {group, playoff} (both in the gate) — so it renders in source; the "dead" report is most consistent with a STALE pre-`c0bc6f6` deploy. Hardened anyway (gate widened to also cover `complete`). **1c — mobile reachability:** added a standalone "Browse players" `<a href="/players">` to the MoreSheet (the mobile nav surface) — NOT a NavId, so /players stays out of the nav model. **A first-class Players bottom-nav tab is T15-2's to add** (it owns the tab bar + the F-P0-A1 nav-tap P0) — see BACKLOG.

**Decision (f) — count is correct by construction (no filter).** DIAGNOSIS: `count(*) FROM player = 1272`; `loadPlayers.player.findMany` has no `where` → returns all 1272. The live "422" = players on non-eliminated teams (the Active-teams-ON subset), NOT a loadPlayers filter. (Aside: only 32 teams carry `eliminated=true`, so `nationAlive` marks 80 teams "alive" incl. non-WC teams — an ingestion/derivation matter, OUT of scope.)

**Decision (g) — the shared card is NOT rebuilt (item 6).** The card's Stats tab already has the per-match Match log (`Completed matches`) + tiles, and the Points tab the breakdown → it matches design image 3's content (Points breakdown · Stats · Match log). Not materially short → reused unchanged (it is /waivers' card too; its suite stays green).

**Invariants + gate.** BYTE-UNTOUCHED: `packages/faab`, bid routes, `BidComposer`/`FreeAgentPanel` submit internals, scoring/recompute/ingest/lock-on-play/auth; NO migration/schema/RLS/Realtime; `design/design_reference/` frozen; `/waivers` flow byte-equivalent (its suite is the proof — the browse link move is presentational). Full DoD gate green (typecheck/lint/format/**3237 passed / 104 skipped**/`@app/web` build, `/players` = ƒ) + **`verify-players.mjs` 11/11** across desktop 1180 + mobile 360/390 (statline + flag-kit + full count + tappable tile + closed-window waivers link + MoreSheet entry) with screenshots; `theCutSkin.test.ts` green (19). See PROJECT.md → 2026-07-04 (PLAYERS-1 remediation) + ARCHITECTURE §29 + BACKLOG → PLAYERS-1/PLAYERS-YC/T15-2. Related: [[players-browser]].

## 2026-07-04 — PLAYERS-TAB: `/players` is a first-class nav tab (ungated), reversing the PLAYERS-1 URL/tile-only reach (`feat/players-nav-tab`, contained nav-config, **MERGED `--ff-only` + DEPLOYED**)

**Decision.** Execute the T15-2 handoff for the `/players` tab now, as its own contained thread: promote `/players` from URL + dashboard-tile + MoreSheet-only reach (PLAYERS-1 / §29.1, which deliberately left it OUT of the `NavId` model) to a **first-class, UNGATED nav tab** on BOTH surfaces — the desktop top strip (immediately after Waivers, its acquisition companion) and the mobile bottom bar. Modelled on the `COMMISH_NAV_ITEM` precedent, but always-render (not `is_commissioner`-gated).

**Mechanism (config-only).** One shared `crossNav` config: add `players` to the `NavId` union + a `NavItem` in `NAV_ITEMS` (desktop) and `BOTTOM_TAB_ITEMS` (mobile), and keep it OUT of `MORE_SHEET_ITEMS` — that non-membership is precisely what makes `selectMobileNavPartition("players")` return `bottomActive:"players"` (a primary bottom tab) rather than a More-overflow entry. `AppShell` needed only the type-forced `NavIcon` glyph (the exhaustive `Record<NavId,ReactNode>` makes a missing key a `tsc` error); it already renders both surfaces from the arrays with `is-active`/`aria-current`. `navItemsForPhase` leaves `players` unrelabeled → present in every phase. `players/layout.tsx`: `active={null}`→`active="players"` (the §29 `NavId|null` widening still stands for genuinely nav-less shell screens).

**Boundary.** The mobile bottom bar goes to 5 tabs + More (6 slots). The tighter 4→5 spacing AND the **F-P0-A1** bottom-bar tap-reliability fix remain **T15-2's** — this thread added the item + wiring only and did not touch that fix. The §29.1 MoreSheet "Browse players" entry is kept byte-identical as a redundant fallback. `/players` route/`loadPlayers`/statline/shared card/scoring/FAAB byte-untouched; no migration/schema/RLS/Realtime.

**Gate.** Full DoD gate green (typecheck/lint/format/**3246 passed / 104 skipped**/`@app/web` build, `/players` = ƒ) + **`verify-players.mjs` 14/14** — the render proof now asserts, on desktop AND mobile, that the Players tab is present on the visible nav surface, active-highlighted (computed style distinct from a sibling), and **taps through** to `/players` (a click navigates the browser; presence alone is insufficient), via a nav replica pinned to source by `playersRenderProof.test.ts`. No DB/RLS/scoring ⇒ no gated-PG run. Contained + PRE-CLEARED → **MERGED `--ff-only`**. See PROJECT.md → 2026-07-04 (PLAYERS-TAB) + ARCHITECTURE §29.2 + BACKLOG → PLAYERS-TAB/T15. Related: [[players-browser]].

## 2026-07-04 — PLAYERS-DATA-scope: scope `/players` to WC participants via the `fifa_match` schedule — a defensive guard, premise corrected against live data (`feat/players-wc-scope`, contained read-scope, **MERGED `--ff-only`**)

**Premise correction (recorded first — the task's stated premise was disproven by a direct prod query).** The thread was scoped on the belief that the 49 non-WC `fifa_team` rows inflate the `/players` pool and the Active-teams toggle over-marks ~80 teams "alive." A read-only ground-truth query disproves this AT THE `/players` SURFACE: all **1,272** players already sit on scheduled teams (the pool spans exactly **48** distinct teams ⊆ the **63** in the schedule; the 49 non-scheduled teams carry **0** players; 0 null-team). So scoping the pool removes **0** rows — a NO-OP on today's data — and the "~80 alive" figure (112 − 32 eliminated) is a `fifa_team`-level artifact that never reaches the player pool. Chat re-authorized proceeding as a defensive guard with the acceptance criteria corrected (drop "N < 1,272" / "non-WC player excluded"; assert the TRUE invariant instead).

**Decision — the participant predicate.** A team is in the tournament **IFF its id appears as `home_team_id` OR `away_team_id` in `fifa_match`**. The loader derives `participantTeamIds = home ∪ away` (non-null) from ONE read-only two-column `fifaMatch.findMany` over the **full** schedule, and filters the pool with `scopeToParticipants` (keep `teamId ∈ set`; null-team excluded). Rows build from the scoped list, so `nationAlive = !eliminated` can only ever mark participants.

**Why NOT `group_id` or `eliminated` (the two tempting-but-wrong signals).** `fifa_team.group_id` is **NULL for all 112 teams** — it carries no membership information at all. `eliminated` marks only knocked-out teams (32 set), so "not eliminated" is TRUE for the 49 non-WC nations too — using it as a membership signal is exactly the over-marking bug. The schedule FKs are the only column that distinguishes participants.

**Why the FULL schedule (no `status` filter).** An eliminated team has only *completed* matches; its players are still WC participants and must remain in the pool (greyed "out"). The existing status-filtered `fifaMatch` read (scheduled/in_progress, for the kickoff-cutoff clock) would shrink the participant set as matches complete and wrongly drop those players — so the participant read is a SEPARATE, unfiltered read ("who is in the tournament" ≠ "what's still acquirable").

**Why a guard, not incidental.** Today's no-op is a property of current data, not a guarantee: the roster feed already knows all 112 teams (it populated the 49 empty rows), so a future ingest could attach players to a non-WC team. The scope filter makes "the `/players` pool contains only WC participants" an ENFORCED invariant — unit-pinned (`playersLogic.test`: a synthetic non-participant IS excluded) and DB-proven (`verify-players.mjs`: pool ⊆ participants, ≤63 backing teams) — rather than an incidental fact that a feed change could silently break.

**Why 63 (the schedule set), not 48 (the real bracket).** The pool spans 48 teams and the schedule carries 63 — the extra 15 scheduled teams hold no players (never reach `/players`). Narrowing the predicate to a clean 48-team bracket is **NOT** attempted: there is no fixture-type / round column to separate bracket fixtures from the rest non-circularly (deriving "the 48 with players" from "teams with players" is circular and would silently drop a real team the instant its players are late-ingested). The 63-vs-48 gap is filed as an OPEN note (BACKLOG), not closed here.

**Invariants + gate.** READ-scope only: no `fifa_team` write, no `eliminated`/worker/FAAB/scoring/ingest touch, no migration/schema/RLS/Realtime; `/waivers` byte-unchanged. Scope fence held to `loadPlayers` + its two tests + `playersLogic` + `verify-players.mjs`. Full DoD gate green (typecheck/lint/format/**3251 passed / 104 skipped**/`@app/web` build, `/players` = ƒ) + **`verify-players.mjs` 15/15** (`N=1272 · 48 backing teams · 63 participants · 0 off-schedule`). Contained read-scope + Chat-authorized → **MERGED `--ff-only`**. See PROJECT.md → 2026-07-04 (PLAYERS-DATA-scope) + ARCHITECTURE §29.3 + BACKLOG → PLAYERS-DATA-scope. Related: [[players-browser]], [[players-nav-tab]], [[auto-team-elimination]].

## 2026-07-04 — T15-2: ONE z-scale for the whole app — scrims above nav, nav inert under scrim (`feat/shell-stacking`, global chrome, **MERGED `--ff-only` after clearance**)

**Context.** The audit's F-P1-I1: the fixed mobile bottom nav (z-100) painted OVER every route modal scrim except the score sheet (pool 50 · pc-scrim 80 · draft toasts 80 · wv-scrim 90 · wv-card 95) and stayed TAPPABLE above open dialogs — a mid-bid tap on a tab silently navigated away. Three surfaces had already independently solved it locally (score sheet z-200; The Cut drill-in/ceremony 110/120; `/players` route-scoped 120 "per the T15 rule") — the rule existed as folklore, not as a system.

**Decision (a) — the ladder is FOUR tokens in ds.css `:root`, and ad-hoc z values between them are defects:** `--z-nav:100` (shell chrome; 101/102 reserved for the shell's OWN More backdrop/sheet — they must beat the bar but stay below route overlays) `< --z-overlay:120` (every route modal/sheet scrim + floating toasts/dropdowns) `< --z-overlay-stack:130` (a second overlay opened ON TOP of an open overlay — concretely the FA card launched from inside the bid composer; ties at one tier would leave stacking to DOM order) `< --z-takeover:200` (score sheet/forfeit). The Cut's 110/120 already sit inside the ladder and stay LITERAL — T15-CUT surfaces are fenced byte-untouched this thread (`verify-the-cut.mjs` 43/43 UNMODIFIED is the proof). **The four per-route ds.css copies changed in lockstep** (they load after the canonical copy — fixing only `app/styles/ds.css` would be silently re-overridden; `appShell.test.ts` byte-identity keeps this honest). The `/players` pioneer override is retired (the base token covers it).

**Decision (b) — the nav is pointer-inert while any route scrim is open**, via `body:has(.pc-scrim, .wv-scrim, .pl-modal-overlay, .sl-forfeit-overlay) .sh-btmnav { pointer-events:none }` — belt-and-braces on top of the z-order (the scrim already covers the bar). The More backdrop/sheet are deliberately NOT in the list (they render INSIDE `.sh-btmnav` — deadening the bar would deaden them; the backdrop covers the bar at 101), and the ko surfaces are excluded (fenced; they cover at 110/120).

**Decision (c) — F-P0-A1 is multi-factor; the in-scope fix is FEEDBACK + GEOMETRY, and the latency residual is REPORTED, not smuggled in.** Diagnosis (pre-fix, in `audit/T15-2_SHELL_STACKING_NOTES.md`): primary = zero tap feedback on MPA `<a>` tabs (tap-highlight suppressed, no `:active`, server-rendered highlight, no loading UI) × live-Render TTFB; plus the 6-slot min-content squeeze (nowrap + `min-width:auto` → "More"/"Players" under 44px, viewport overflow ≤~340px); plus two runtime-only iOS contributors (toolbar-restore band; T15-3's input-zoom viewport offset). Fix: `:active` press state + `touch-action:manipulation` + equal slots (`flex:1 1 0; min-width:0` + label ellipsis guard + `min-height:44px`). **Accepted trade-off:** the long GROUP-phase label "Vs the field" can ellipsize on phones — the live phase is knockout ("The Cut" fits) and the label set is fenced product config. **NOT fixed here (needs its own clearance):** `next/link`/prefetch or a navigation progress affordance for the MPA latency itself — routing behavior, not chrome.

**Decision (d) — ONE sheet-behavior kit, not five hand-rolled variants:** `useSheetChrome(open, onClose, panelRef)` = stacked-COUNT body-scroll lock (the FA-card-over-composer stack must not unlock the body when the top sheet closes) + Escape (latest-handler ref) + focus trap + opener focus-restore, wired into all six sheet/modal surfaces. CSS side: `overscroll-behavior:contain` on MODAL scrollers only — the in-flow FA panel reuses the same `.wv-comp-*` classes and MUST keep chaining page scroll (containment there would eat the page gesture; the modal instances are scoped `.wv-scrim .wv-comp-*`). All modal caps vh→dvh; scrims get `env(safe-area-inset-*)`.

**Decision (e) — step-27 is a LAYOUT-ORDER fix, not a z fix.** The instant-pickup composer is an in-flow `<section>`, not a modal: its confirm row sat under the nav while the 420px pool list + 220px drop-picker consumed every vertical gesture, so page scroll never revealed it. At ≤768px the action column (Adding · drop-picker · confirm) orders ABOVE the pool list (`.wv-fa`-scoped `order:-1`) — reachable under the panel header with zero scroll-threading; the SEALED-BID modal keeps pick→config order and instead scrolls internally (`.wv-comp-body` overflow at ≤768px, which also un-strands the Place-bid row inside the dvh-capped, `overflow:hidden` composer).

**Decision (f) — harness-found production bug, fixed with the proven sibling pattern:** `.wv-composer{width:780px; max-width:100%}` in the grid-centered scrim rendered 780px wide at phone widths (percentage max-width resolves against the auto grid track that the fixed width itself sized), pushing ✕ + Place-bid off-screen right → `width: min(780px, 100%)` (the `.pl-modal` form, proven in the identical scrim).

**Verification model.** Geometry/paint-order truths in a REAL browser: new `verify-shell-stacking.mjs` (pw.local replica, real CSS inlined; 33/33 at 360/390/430 + desktop — overlays above nav with clickable chrome, nav tap-proof under scrims via a REAL dispatched click, SaveBar clear of the band, step-27 reachable, tap-through on every tab + a More item, 6 equal ≥44px slots, no h-overflow, bar ≤58px). Byte-level declarations pinned in the vitest gate: `shellStacking.contract.test.ts` (17); behavior wiring in jsdom: `moreSheetChrome.dom` (6, mounts the REAL MoreSheet) + `useSheetChrome.dom` (3, stacked-lock count + latest-onClose + no re-lock churn). Gate: typecheck/lint/format/**3277**/web build; the five pre-existing verify scripts green, the two fenced ones UNMODIFIED. **MERGED `--ff-only` (2026-07-04, Sergio cleared merge + push + deploy)**; **F-P0-A1 CLOSED 2026-07-05** — Sergio's live Render eyeball (screenshots `apps/web/screenshots/shellstack-*.png`) came back clean and the on-device verdict was A (see 2026-07-05 decision below). Related: [[t15a-mobile-audit]], [[players-nav-tab]], [[t15-cut-the-cut-layer]].

## 2026-07-04 — NAV-LAT: route `loading.tsx` skeletons make slow MPA navigation visible (`feat/nav-latency-feedback`, additive UI, **MERGED `--ff-only` + DEPLOYED**)

**Context.** The T15-2 F-P0-A1 residual (SHELL_STACKING_NOTES §5/§6): the bottom-tab / More destinations are plain server-rendered `<a>`s with **zero `loading.tsx`**, so on live-Render TTFB a tap paints nothing for seconds and reads as a dead tap (walkthrough N1; compounds step 7's tab-switch freeze). T15-2 shipped the touch-down `:active` press; this decides the content-level companion.

**Decision (a) — add route-level `loading.tsx` skeletons, styled from the ONE existing global `.skeleton` shimmer, no new CSS, no JS.** A shared `app/shell/RouteSkeleton.tsx` (pure server component; inline token styles + the `ds.css` `.skeleton` class, dark-correct) with six archetype variants (`list`/`pitch`/`cockpit`/`board`/`form`/`dashboard`), each leading with a header band over the body shape so the real page paints INTO the same frame (no layout-shift jump). Rationale: on a hard MPA nav, streaming SSR flushes the shell + skeleton FIRST and streams slow content behind it, so the destination starts painting structure once the response opens. This is a `additive UI` layer only — `shell.css`/`ds.css`/`crossNav.ts` arrays and every loader/API/schema/RLS/Realtime are byte-untouched.

**Decision (b) — the skeleton nests inside the layout's AppShell for the 11 layout-mounted routes; `/` gets its OWN static shell chrome.** For lineup·vsfield·pool·players·standings·waivers·scoring·settings·playoffs·draft·commish the AppShell is mounted in the route **layout**, so `loading.tsx` (the page-segment Suspense fallback) renders inside `.sh-content` and the shell + bottom nav + active-tab highlight survive for free. `/` mounts AppShell inside `page.tsx`, so its fallback would lose the shell — its `app/loading.tsx` therefore renders its OWN static, **inert** chrome (Dashboard active), deliberately NOT the async `<AppShell>` (an async fallback would suspend and defeat the point). The home chrome is a hand-mirror, so it is pinned to AppShell source (Decision d).

**Decision (c) — `/games` is out of scope and it is a coverage question, not an omission.** Confirmed against `crossNav.ts`: there is no `games` id in the `NavId` union or in `BOTTOM_TAB_ITEMS`/`MORE_SHEET_ITEMS`, and there is **no `/games` index route** (`app/games/` holds only `[matchId]/`). `/games/[matchId]` is reached ONLY by in-screen deep link (dashboard/vsfield/pool fixtures), never the bottom bar or MoreSheet, so it falls outside the brief's "reachable from the bottom bar and the MoreSheet" clause. Recorded so a later thread does not re-litigate it.

**Decision (d) — the home hand-mirror is pinned to REAL AppShell source, not to itself.** Because `app/loading.tsx` re-emits AppShell's nav classes by hand, `routeSkeleton.dom.test.tsx` reads the REAL `AppShell.tsx` / `MoreSheet.tsx` source and asserts a shared `SHELL_VOCAB` (`sh-app sh-app-top`, `sh-topbar`, `sh-topnav-scroll`, `sh-topnav`, `sh-nav-item`, `sh-content`, `sh-btmnav`, `sh-btnav-item`, `is-active`, `sh-more-btn`) is present in BOTH the source and the loader — so a future rename in AppShell breaks the test rather than silently diverging the loading chrome (the `shellStacking.contract` / `verify-players` byte-pin precedent; verified non-vacuous).

**Decision (e) — `<Link>`/prefetch is ANALYZED, NOT decided here (clearance still HELD).** `loading.tsx` on a hard MPA nav only helps AFTER TTFB; the change that makes it fire INSTANTLY on tap is client-side `<Link>` navigation. Full write-up in `audit/NAV_LATENCY_NOTES.md §5` (AppShell can stay a server component; `<Link>` degrades to MPA pre-hydration so the 5 anchor slots are NOT dead-until-hydration, only the More `<button>` is; prefetch warms only the force-dynamic boundary but the fixed bar keeps all 5 Links in-viewport → start `prefetch={false}`; no shared authed nav layout exists → a Link transition re-renders each AppShell + re-runs `loadNavPhase()` per nav, a separate refactor). Recommendation is to convert, but it is a routing-behavior change and its clearance is separate from this merge. **F-P0-A1 CLOSED + Link-conversion clearance CLEARED 2026-07-05** — see the 2026-07-05 decision below; the `<Link>` conversion recommended here is now cleared to build as its own thread.

**Verification model.** `verify-nav-latency.mjs` (48/48, Playwright): renders the REAL `RouteSkeleton` (transpile → `renderToStaticMarkup`, zero non-type imports → no replica drift) + drives active-state via the REAL `crossNav` selectors; a streaming HTTP server (flush shell+skeleton → hold → stream content) proves the skeleton paints BEFORE content at 390+desktop, shell/nav visible + active, no shift, no h-overflow; screenshots `apps/web/screenshots/nav-latency-*.png`. Plus `routeSkeleton.dom.test.tsx` (30/30) mounts the real components incl. the source-drift pin (Decision d). Gate: typecheck/lint/format/**3307**/web build; fence verifiers UNMODIFIED (verify-the-cut 43 · verify-playoffs-hero · verify-players 14 · verify-shell-stacking 33/33). **MERGED `--ff-only` + DEPLOYED (2026-07-04, Sergio-cleared merge+push+deploy — the T15-2 precedent)**; Sergio's live eyeball (home/lineup/standings skeleton screenshots) came back clean. Related: [[t15-2-shell-stacking]], [[t15a-mobile-audit]].

### 2026-07-05 — F-P0-A1 closed (verdict A) + NAV-LAT Link-conversion clearance granted (docs-only, Sergio's live on-device pass)

**Decision.** Sergio's on-device live pass (owed since T15-2 + NAV-LAT, 2026-07-04) returned: skeleton visual pass **PASS, no issues** on every route (no Safari-vs-PWA discrepancy); F-P0-A1 verdict **A — closed** (the `:active` press state registers on every tap and navigation completes; no residual dead-tap case survived). **F-P0-A1 is now CLOSED**, superseding its prior "mitigated, on-device verdict pending" status everywhere it was recorded (BACKLOG T15/NAV-LAT rows, PROJECT.md T15-2/NAV-LAT session entries, the two decisions above).

**Link-conversion clearance: CLEARED.** The `<Link>`+prefetch conversion analyzed (not built) in `audit/NAV_LATENCY_NOTES.md §5` is cleared to proceed as its own thread — no change made here, clearance only.

**Scope.** Docs-only: BACKLOG.md, PROJECT.md, DECISIONS.md (this entry + the two amended above), `audit/NAV_LATENCY_NOTES.md` §4/§6, `audit/T15_WALKTHROUGH_RESULTS.md` operator-queue. Zero code/test/CSS/config changes. Commit `[skip render]` — no Render deploy. Related: [[t15-2-shell-stacking]], [[nav-lat-loading-skeletons]].

## 2026-07-05 — NAV-LINK: bottom-nav Links carry `prefetch={false}` (conscious posture, not a default) (`feat/nav-link-conversion`, additive UI, **MERGED `--ff-only` + DEPLOYED** after the on-device gate)

**Decision.** The 5 bottom-tab `<a>`s (`AppShell.tsx`, `.sh-btmnav`) and the MoreSheet item `<a>`s (`MoreSheet.tsx`) convert to `next/link <Link>` — the `audit/NAV_LATENCY_NOTES.md §5` recommendation — but with **prefetch DISABLED** on every converted Link. Rationale: the fixed bottom bar keeps all 5 tab Links in-viewport on every authed screen, so App Router's default eager prefetch would warm all 5 `loading.tsx` boundaries on every render, re-warmed as the user navigates — pure amplification. Because every authed route is `ƒ` force-dynamic, prefetch fetches only the static shell-up-to-Suspense boundary (not page data), but the always-in-viewport multiplier makes even that wasteful on metered connections; `prefetch={false}` fetches on hover/touchstart instead. Tunable back to eager if measurement wants it — the point is the choice is **recorded, not defaulted** (commented at both sites citing §5).

**Why it's low-risk (the analysis that cleared it).** `<Link>` renders a real `<a href>`, so pre-hydration taps degrade to plain MPA navigation — the 5 slots are never dead before hydration (only the More `<button>` is JS-gated, unchanged). AppShell stays a **pure server component** (`<Link>` ≠ `"use client"`; verified — no `"use client"` added). Active-highlight logic is unchanged (explicit per-route props, no `usePathname`). The payoff: post-hydration the App Router intercepts the tap → client-side transition → NAV-LAT's `loading.tsx` fires INSTANTLY (collapsing the dead-tap window), instead of only sooner-after-TTFB.

**Corollary decision — `playersRenderProof.test.ts` retarget.** That test pinned the literal `<a href="/players" className="sh-more-item"` MoreSheet markup. Since the conversion is intentional and the test's *contract* (a MoreSheet entry linking to /players) is unchanged, the assertion was **retargeted** to the `<Link href="/players" prefetch={false} …>` markup rather than deleted — a source pin tracking a deliberate markup change (the `shellStacking.contract`/`verify-players` precedent). A new source-drift `describe` in `routeSkeleton.dom.test.tsx` (5 cases, non-vacuous) additionally pins the production shell source so a future revert to `<a>` breaks the suite.

**Held for a separate future thread (§5 point 4).** No shared authenticated route-group layout exists, so each client transition re-mounts the route's own `<AppShell>` and re-runs `loadNavPhase()` per nav (the highlight still moves correctly and instantly; the nav is simply not *persistent* across transitions). A truly persistent, non-re-rendering nav is a larger structural refactor (relocating 11 layouts' shell mount), held out of this step.

**On-device gate + verification.** Sergio's live on-device pass **PASSED, no findings** (the F-P0-A1 closing-evidence precedent — the clean live pass is the evidence). `verify-nav-link.mjs` **14/14** (real Next 15 fixture for the client transition + instant skeleton + real-`<a>` SSR; class-faithful replica for MPA tap-through + active-highlight + MoreSheet nav + no h-overflow). Gate: typecheck/lint/format/**3312**/web build (every authed route still `ƒ` dynamic); fence verifiers UNMODIFIED (the-cut 43 · playoffs-hero · players 14 · shell-stacking 33/33 · nav-latency 48/48 · mobile-nav). **MERGED `--ff-only` + DEPLOYED (2026-07-05, `main@909cecf`; Render web deploy fired + healthy — new build hash live/stable, `/api/health` 200).** Related: [[no-reopening-spec-pinned-decisions]], [[t15-2-shell-stacking]], [[mobile-bottom-nav-layer]].

## 2026-07-05 — T15-7: the `/scoring` rulebook renders from local data tables + engine-derived fixtures, probed against the engine (`feat/scoring-rulebook-truth`, trust surface, **MERGED `--ff-only` + DEPLOYED**)

**Decision.** The `/scoring` reference page must never again silently diverge from `packages/scoring`. Rather than re-hand-authoring the corrected tables, the page's drift-prone numbers now live as **local data** in `apps/web/src/scoring/scoringData.ts` that the page renders and the test probes against the engine: §1 as `RATING_LADDER`, §4 as `ACCUMULATORS` (with `{field, per, sign, eligible}`), §8 as `RED_CARD_BANDS`/`SECOND_YELLOW_BANDS` + `YELLOW_CARD_PTS`/`OWN_GOAL_PTS`. The four §9 worked cards are **derived**, not authored: each is a `ScoreInput` fixture pushed through `scorePlayerMatch`, so its rows and total ARE the engine's output.

**Why this shape (and what was NOT done).** The audit (F-P1-J1/J2/J3) found §1 bands mislabeled + the 0-band missing, §4 possession-lost at ÷3 instead of ÷10 plus five categories absent, and §8 red one point harsh in every band — and the old `scoringData.test.ts` couldn't have caught any of it, because it only checked each card's rows summed to a *hand-written* total (never against the engine). The fix therefore had to close that gap, not just correct values. `scoringData.test.ts` now probes every §1/§4/§8 displayed value and every §9 row/total through `scorePlayerMatch` — so **page-vs-engine drift fails CI**. The alternative — an exported `RULES` manifest inside `packages/scoring` generating §1–§8 from engine constants — was **explicitly DEFERRED** to its own later thread: it would mean lifting function-internal literals (the `ratingPoints`/`redCardPoints` ladders, inline `floorPer` divisors) into an exported surface inside the purity-critical, fence-critical engine package, whose whole value is that it never churns. The local-data-table + probe-test approach kills drift for practical purposes without touching the engine (`packages/scoring` byte-untouched; read-only pure import of `scorePlayerMatch` only).
...
### 2026-07-05 — T15-15 (PII-GUARD): email-shaped `manager.displayName` rejected at both write paths (merged `c2b0150`, deployed)

**The leak, and why a write guard.** `manager.display_name` is a display-only column, but email-shaped values (typed into the provisioning config for members whose preferred name the commissioner didn't have) leaked member emails across every surface that renders a manager name (T15-13 N2/N6). T15-14R traced every writer and confirmed the origin is **config-borne**: the provisioning upsert is the SOLE creator of manager rows, and its only `displayName` check was non-empty. The self-serve rename is the second (recurrence) writer. A write-path guard at both — rejecting an email SHAPE — makes the leak unable to recur; the existing polluted rows are a separate backfill (still OPEN — see below).

**The predicate (the crux).** One shared pure `looksLikeEmail(value)` in `@app/shared` — full-string email match on the trimmed value (`/^[^\s@]+@[^\s@]+\.[^\s@]+$/`). **Full-string only**: a legit name that merely *contains* `@` (e.g. `n@cho`) stays legal — real names have apostrophes, accents, stylized characters. `[^\s@]` forbids internal whitespace and stray `@`, so trim is the only normalization needed. Pinned by a 17-case table test (`n@cho`/`@handle`/`me@`/`a@b`/`Nacho @ Home`/`a b@c.co`/plain names → allow; whole addresses + whitespace-padded → reject).

**The two gates.** (1) `validateConfig` (`apps/worker/src/provision/plan.ts`) pushes a loud validation error on an email-shaped `displayName` — **reject, not silently normalize**: the config is operator-authored and re-runnable, so a loud error at plan time beats a silent rewrite. (2) `validateDisplayName` (`apps/web/src/manager/displayName.ts`) gains `reason: "email_like"`; the route already surfaces `validated.reason` as the 400 body, so no route change.

**Live-safe, no contract touch.** Display-only column; every viewer/"You" check compares manager **ids**, not names, and all name maps are id-keyed (T15-14R §4) — nothing keys on the name, so the guard changes no behavior beyond blocking bad writes. Scope = 7 files (predicate + 2 guards + tests + export). **No DB, no migration, no render site, no loader, no `packages/scoring`, no fence verifier touched.** DB-level CHECK constraint deliberately NOT added — a mid-tournament migration for a display column is over-engineering when two app-level gates cover both writers (revisit only if a third writer appears). TDD red→green; full DoD gate green (typecheck/lint/format/**3382 tests**); all fences green unmodified.

**Recurrence vector CLOSED; backfill still OPEN.** Existing email-shaped rows persist until the slot-keyed backfill (T15-14R §3a) runs — gated on Sergio's prod counts (queries 1–5) + slot→real-name map + HARD-1 authorization (writes live data). Tracked as `PII-BACKFILL` in BACKLOG. Related: [[t15-14r-displayname-root-cause]], [[profile-rename]] (the rename validator + unique index), [[provisioning-deploy]] (the config→upsert seam), [[allowlist-seed-layer]] (the committed-constant-script pattern the backfill will follow).

### 2026-07-05 — `manager.id` is TEXT (uuid-shaped), NOT `@db.Uuid` — raw-SQL id binds must be bare, never `::uuid`

The T15-BACKFILL runner threw Postgres `42883` (operator does not exist: text = uuid) three redeploys running because the guarded `WHERE` cast the `id` bind as `$n::uuid`. Root cause: `manager.id` is a **TEXT** column holding uuid-shaped strings — it is **not** typed `@db.Uuid` in the Prisma schema. So any raw-SQL predicate on `manager.id` must bind the parameter **bare** (`id = $n`), never `id = $n::uuid` — the cast makes the column-side `text` mismatch the param-side `uuid`. The fix is single-sourced in `guardedWhere` (one shared fragment across the dry-run preview + the `UPDATE`), so both paths bind identically and can never drift. Applies to every future raw-SQL touch of `manager.id`. Related: [[t15-14r-displayname-root-cause]].

### 2026-07-05 — Schema-wide: EVERY uuid-shaped id/FK column is TEXT — raw-SQL id binds are always bare `$n`, never `::uuid` (fence-enforced)

**Generalizing the `manager.id` rule to the whole schema.** The `manager.id`-is-TEXT landmine above is **not a special case — it is the universal case.** The SCHEMA-UUID-AUDIT (`audit/SCHEMA_UUID_INVENTORY.md`, origin/main `4591963`) inventoried every id/uuid-defaulted column in `packages/db/prisma/schema.prisma` and found **zero `@db.Uuid`** anywhere: every `String @id @default(uuid())` PK, every bare-FK PK, every composite-PK part, and every relation FK (`leagueId`, `managerId`, `userId`, `playerId`, `teamId`, `matchId`, `periodId`, …) is a Postgres **TEXT** column holding a uuid-shaped string. `String @default(uuid())` generates a v4 uuid *application-side* but maps to `text` unless `@db.Uuid` is added — and it never is.

**The rule (schema-wide invariant).** In any hand-written raw SQL (`$queryRaw*` / `$executeRaw*`), bind id/FK params **bare**: `WHERE id = $n`, `WHERE "fooId" = $n`. **Never** append `::uuid` to an id column or its parameter — the cast makes the param-side `uuid` mismatch the column-side `text` and raises `42883` (the exact T15-BACKFILL bug). For array binds use `= ANY($n::text[])`, **never** `::uuid[]`. Prisma's own generated queries are parameterized as text and are unaffected; the hazard is **only** in raw SQL that adds an explicit `::uuid`.

**Prevention, not repair — enforced in CI.** The audit found **zero `::uuid` id-bind offenders** on any path today (the sole live-write raw-SQL id bind, `packages/db/src/backfillDisplayNamePiiRunner.ts`, already binds bare and is PG-proven at `97649aa`). To keep it that way *before* the §5 launch-audit backfills start writing raw SQL against prod, a CI-gating vitest fence — `apps/web/src/fences/uuidBindFence.test.ts` — greps product source (`.ts`/`.tsx`, tests excluded) with comments stripped and **fails on any `::uuid` in executable raw-SQL code**, plus a non-vacuity probe that must locate a real raw-SQL id-bind site (a moved/emptied tree can't green-scan nothing). `::uuid` is a SQL-only construct with no TypeScript meaning, so any occurrence in non-comment code is necessarily inside a SQL string — the predicate needs no call-site tracking. The `auth.uid()` RLS shim (`…::uuid::text`) is deliberate auth-path canonicalization, **not** an id bind; it lives only in integration **tests** (outside the product-source scan) — excluded by construction, with no product-source `auth.uid()` executable site existing. Migration `.sql` and comments (the audit's benign `::uuid` prose) are not scanned. The fence rides `pnpm test`, so it gates CI and the DoD gate automatically — the §9 engine-probe / time-truth-fence analog for the id-bind class.

**No schema change.** Migrating the ~35 TEXT-uuid columns + every FK to `@db.Uuid` (audit Option B) is migration-class, mid-tournament, whole-schema, touches every RLS policy and the `auth.uid()` text-vs-uuid contract, and buys nothing functional (Prisma already works as text). Deferred indefinitely / post-launch. The convention + fence (Option A) is the accepted standard. Related: [[t15-14r-displayname-root-cause]] (the `manager.id` precedent), [[rls-model]] (the `auth.uid()` shim contract), [[t15-6-time-truth-fix]] (the vitest-fence-rides-CI precedent).

### 2026-07-06 — D-MEM / D-COMM / D-ROUND: three tier-0 model decisions settled (D-DB remains open)

**D-MEM — membership model: user × league join table.** One user belongs to N leagues; the single `manager.user_id` model is rejected. Cross-refs F-C03/C08/C13/C14. MT-1 builds membership as an N:N join + allowlist rework.

**D-COMM — commissioner model: per-league, scoped writes.** No global super-commissioner; each commissioner's authority is bounded to their own league. Cross-refs F-C04/C06/C07/C19. MT-1 builds scoped commish writes.
...
### D-IDR — Identity-arc ratification (2026-07-07, commissioner): D1–D8 adopted
All eight ratified as recommended (chat record 2026-07-07); settle applied by Design same day (spec → v5, canvas recaptions + fixes C1–C10, per-edit confirmations on record). Anchors: ROUTES_TRUTH.md @ 9c16e3b · QA snapshot design/canvas-archive/2026-07-07/DESIGN_QA_turns1-10.md @ a55ad2f (relocated 8d53cc7) · FL-1×flip + MG-4 verified from t12 canvas source (sha d416ff23…) · v5 settle bundle archived under design/canvas-archive/2026-07-07/v5-settle/.
- D1: /vsfield consolidation ratified; /playoffs retires. Target IA for the UCL retarget (UCL-4 / Window B); the live WC league's /playoffs is untouched until then. R2 confirmed from source (Dashboard.tsx:351 already deep-links /vsfield).
- D2: standing rule — every color-carried state renders a non-color signal (icon/label/shape/position); release gate on new surfaces. Settle retrofit: one addition (6a selected-pick ✓ prefix); all other v4 surfaces verified compliant.
- D3: §6.2 amendment — self manager disc cobalt + mandatory "YOU"; all others slate (CH-2/MF-1). Seed tags canonical "Sn" (MF-2).
- D4: t11c football backdrop absorbed as visual-contract-behind-gates: UCL-1 schema · 2c leg/tie fork (game detail only) · feed adapter (INV-1). F-P3-B2 answered: matchday module present under league phase, folded.
- D5: §11 aggregate ban narrows — bracket nodes may ship one aggregate + leg-context sub; ban persists for game-detail/match headers.
- D6a: erratum — monogram auto-contrast = luma (0.2126r+0.7152g+0.0722b, raw sRGB) at .55; spec wording corrected, mocks already comply (source-verified).
- D6b: monogram char count resolver-owned (monoText from sizePx): 3ch ≥24 / 2ch <24; call-site free choice removed (MG-1 root cause). Known settle residuals (Design self-report, queued ERR-D6B): t11 resolver + sub-caption still read "3ch≥20px"; 6a desktop rows + 9a hero at 24px still render 2ch; one stale "post-review" annotation.
- D7: t12 absorbed — badge construction v1 enters spec (§4.1), OWNED assets only; real-club posture unchanged (crestUrl:null indefinite; real clubs monogram until licensing). At the t12 snapshot badges were procedural — zero asset URLs in canvas source (verified). The v5 export bundle additionally ships rendered crest/kit assets + support.js for local viewing; archived verbatim, filename whitelist checked against the 36 fictional clubs. Registry flip = canvas baseline; MG-5/MG-6 ratified retroactively. OPEN CONDITION (operator): 36-badge real-club proximity pass — NOR first (name pattern + black/white stripes) — before any badge appears in public captures. Known erratum preserved in the archived QA snapshot: severity tally sums 25 vs the 28-finding count (archive is verbatim by rule).
- D8: micro-batch as drafted — CH-1 red LIVE · ST-1/ST-2 COMPUTED tags · MG-2 ring ≥16 · MG-3 registry-hex fix + result-card identity unified · CH-3 fantasy-value cobalt (text/numerals only) · CH-4 --warn token (hex at token-land) · CH-6 low-alpha state tint (α ≤ .08) · SZ-1/2/3 · FL-1 accepted legacy (source-verified unarmed).
- Token-land carry (HOLD): four chrome hexes + ΔOKLCH gate · --warn hex · CH-5 position-palette collision · monogram-disc shape residual (D2-mitigated; review confirms sufficiency) · NB-1 plinth validation on #0A0D12 · §7.1 OUT-chip --live share.
...
### 2026-07-07 — HARD-1 observability close: Sentry + worker dead-man switch + probe split, all env-gated byte-identical when unset

**What shipped, and why gated-off-by-default.** The launch-audit's F-A01 finding ("no exception/error-tracking service anywhere, every failure is a console line a human has to tail") is closed for the write paths and server loaders that matter most: worker `@sentry/node` (logger warn/error hook + `captureConsoleIntegration` for `@app/ingest`'s raw signals) and web `@sentry/nextjs` (`instrumentation.ts` `register`/`onRequestError` + explicit capture-and-rethrow on `faab/bid`, `lineup`, `draft/pick`). Every integration point is gated on `SENTRY_DSN` being set — unset is a true no-op (worker: `if (config.sentryDsn) Sentry.init(...)`; web: `@sentry/nextjs`'s own DSN-gate) — so local/test/pre-provisioning environments stay behaviorally byte-identical, matching the same-invariant precedent set by `jobs/heartbeat.ts` (A-lite) and the `WORKER_TICK_HEARTBEAT_URL` switch below.

**F-A02 closed — resident-worker dead-man switch.** A-lite (2026-06-29) built cron-stall detection for `wc-fantasy-period-close` only; it structurally cannot watch the **resident** worker process (`apps/worker/src/index.ts`), whose 60s tick loop has no external signal if it hangs or crashes silently. `WORKER_TICK_HEARTBEAT_URL` closes that gap with the identical `safePing`-never-throws pattern, wired inside `tick()` so a hung event loop stops pinging by construction. Deliberately a **separate** Healthchecks.io check from `PERIOD_CLOSE_HEARTBEAT_URL` — they watch different failure domains (hourly cron vs. resident event loop) and conflating them would blind one to the other's outage.

**F-A09/F-A16 — liveness/readiness probe split, not a single healthcheck.** Render's `healthCheckPath` triggers automatic **restarts** on failure, so it must stay dependency-free (`/api/health`) — pointing it at a DB-touching probe would restart-loop the web service on an ordinary Supabase blip, trading a transient DB issue for a self-inflicted outage. Readiness (connectivity + right-schema) lives at `GET /api/db-check` instead, documented in `render.yaml` as the target for an **external** HTTP monitor (UptimeRobot-style) — a wrong-DB deploy or lost Postgres connection pages the operator rather than triggering a restart loop that can't fix a database problem. `db-check` must never be set as `healthCheckPath`.
...
### 2026-07-09 — W2 ratification record: 20 findings settled into spec v6

**(1) Doc-lag resolution — FAAB claim priority.** FAAB claim priority (§D amendment, intra-manager equal-amount tiebreak) **shipped** 2026-07-08 (`main@3b0cae7`, live-fire device gate PASS — see [[faab-claim-priority]]). The `faab-amendment-2026-07-07` memory and the W2 INDEX blocked-list are **stale** on this one point specifically; every other item on those lists stands as previously recorded.

**(2) W2-01 — ANNOTATE, not APPLY.** The canvas row model (stats rendered in-card, NEXT FIXTURE rendered in-row) is the **contract**; the shipped live surface's 9-column+swipe layout is a **workaround**, not the design target. Code should treat the row model as the thing to converge toward on the next `/waivers`-adjacent touch, not as something already satisfied by the current live layout.

**(3) W2-06 — locked semantic: displayed acquisition deadline.** The deadline shown to a manager is the **ACTIONABLE cutoff** — the earlier of (a) the next FAAB batch fire time or (b) the acquisition-target's kickoff-void — **never** a bare per-player kickoff time in isolation. This is now spec-defined (v6, `Handoff Spec.dc.html`), which un-blocks the CutoffTag fix (see BACKLOG.md).

**(4) W2-14 — cadence recorded period-generic.** FAAB batch cadence is spec'd as "per-period batch," not tied to a WC-specific matchday cycle, deliberately for UCL survivability (the same waivers surface must hold for UCL-2's different fixture rhythm without a spec rewrite).

**(5) W2-19 — squad caps format-parameterized.** The squad-cap numbers in the spec (7 starters + 2, i.e. 9-man playoff roster) are WC-tournament values, not universal constants; UCL values are deferred to land in UCL-2 rather than hard-coded into this settle.

**Deferred (spec v6 §12, final Window-B export).** Un-staged W2 rows for surfaces the canvas didn't reach this wave: lineup, pool, games, commish, moresheet, draft-squad. Also deferred: the REVEAL gate, multi-league onboarding (tracked as DEC-0), and two-legged surfaces (carried to Window B's D-ROUND decision). None of these are regressions — they were out of Wave-2's scope, not dropped from it. See [[w2-reconciliation-closeout]] for the archive/ingest mechanics and the POSTURE-UCL1 gate result.
