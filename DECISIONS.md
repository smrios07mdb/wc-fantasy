...
Status lifecycle only (`alive → eliminated`, `alive → champion`). It does NOT trim rosters, touch FAAB (the D4 gate keys on `status==='alive'` so an `eliminated`/`champion` flip auto-removes them — a champion has 0 `alive` entries so FAAB naturally competes nobody), recompute scores (it READS `score_manager_period`), or flip `league.status`. **⚠️ Seam for the next thread:** the `playoff → complete` flip is still deferred (the transition's old comment attributed it to "Phase 2", but this thread's spec scopes it to status-only); functionally harmless (a champion league has no `alive` entries) but the phase label/theater needs it.

### `loadPlayoffs` read contract — DEFINED for the NEXT (UI) thread
The playoff view-model the UI loader will assemble (drawn from `design/design_reference/playoffs/data.jsx` `buildGuillotine`). **Defined here; the loader is NOT implemented in this thread.** See **ARCHITECTURE.md §21** for the full shape. Key points: per-round `{ status: past|live|future, fieldCount, cutCount, survives, ranked: [{ managerId, seed, points, rank, safe|zone|eliminated }], survivors, eliminatedIds }`; the seeds + `seedOf`; the **live/provisional cut line** (computed by running `selectGuillotineCuts` on in-progress round scores — the same pure selector, so the live "facing the blade" zone matches the eventual cut); the viewer's reduced-lineup reference; and the FAAB reinforcement state. The applied (`past`) rounds read straight from `playoff_entry` (`status`/`eliminated_round`); the `live` round is derived (provisional) and the `future` rounds are skeletons (cut counts from `period.cut_count`).
