...
- **Follow-up debt (RECORDED).** (a) There are now THREE mirrors of the cut-apply `$transaction` — the relocated commish-core adapter, the web `commishAdvanceStore`, and the worker `autofire/advanceStore` — pinned against each other by the gated-PG suites but a dedup candidate. (b) The shared `@app/commish-core/advanceAudit` builder is byte-identical to the web `buildAdvanceAudit`+`recordCommishAudit`, but the web copies were left UNTOUCHED (fence); de-duping the web onto the shared builder is a follow-up.
