...
- **Raw-SQL id/FK binds are always bare `$n`, never `::uuid`.** In any `$queryRaw*` / `$executeRaw*`,
  ...
  **never** `::uuid` / `::uuid[]`. Every uuid-shaped column in the schema is Postgres **TEXT** (zero
  `@db.Uuid`), so a `::uuid` cast mismatches the text column → `42883 operator does not exist:
  text = uuid`. See DECISIONS "Schema-wide: EVERY uuid-shaped id/FK column is TEXT" +
  `audit/SCHEMA_UUID_INVENTORY.md`. Enforced in CI by `apps/web/src/fences/uuidBindFence.test.ts`
