# Claude Code — Prompt 16: Landing hub — replace the scaffold `/` with an auth-aware front door + post-login signpost

> Paste into Claude Code with the four brain files in the repo root (PROJECT.md, ARCHITECTURE.md,
> DECISIONS.md, SCORING.md) and Prompts 01–15 in place. **ARCHITECTURE.md §6 (Auth) + the Prompt-07
> `getSessionManager` primitive are the spec for this prompt.**
>
> **Numbering note:** this is **Prompt 16**, deconflicted from the **Prompt 15 already on main** — which
> is the *pre-prod security follow-ups closeout* (mirror-function / `search_path` hardening, migration
> `20260606180000`), **unrelated to this prompt**. Do not touch that work. This prompt closes the
> separate go-live navigation gap from the route-map report: `/` is the unchanged Prompt-01 scaffold
> with no link to sign-in, AND the magic-link callback's post-login redirect defaults to `/`
> (`safeNextPath(null)`), so even an authenticated member lands on the scaffold with no path onward. One
> file fixes both.

---

## Context (read first)
Read **ARCHITECTURE.md §6** + the Prompt-07 status in PROJECT.md. State of the build: Prompts 01–15 are
...
set-lineup flow (10), the live vs-the-field screen (11/13), and the pre-prod security follow-ups (15).
The Render deploy is green (main `9accb1f`; the scaffold `/` is **unchanged** from the `c8f404d`
route-map report — the security closeout that merged on top was `[skip render]` docs + a migration, no
app code). **The only thing missing is the front door:** `apps/web/app/page.tsx` is still the Prompt-01
scaffold ("Foundation is up…" + links to `/api/health` and `/api/db-check`). It has no nav and no link
to `/sign-in`, and because the callback redirects to `next || /` and the magic link carries no `next`,
signed-in members are dropped right back onto that scaffold — `/draft`, `/lineup`, `/vsfield` are
reachable only by hand-typing the URL.
...
- **The Prompt-15 security closeout** — already merged; do not touch the mirror function, the
  `search_path` pin, or the security migration.
...
- No out-of-scope work: no redirect change, no feature-page / API / middleware / auth-core edits, no
  admin surface, no shared cross-nav strip, no security-closeout churn, no new env/routes.
...
commands you verified; and any `TODO(confirm):` left. Report `git log --oneline -1` and `git status`
post-commit; branch off main, conventional commit, no force-push, **hold the merge for Chat's
clearance**. Flag the deferred **shared cross-nav strip** as the recommended next follow-up. Do not
start styling/Design, the cross-nav strip, or any admin surface.
