...
function p(id: string, pos: Position): LineupPlayer {
  return { id, displayName: id.toUpperCase(), firstName: "X", lastName: id, position: pos, country: null };
}
...
const STARTERS = [GK1, DEF1, "def2", "def3", "def4", "mid1", "mid2", "mid3", "fwd1", "fwd2", "fwd3"];
...
describe("ForfeitConfirmSheet — open / cancel / confirm lifecycle", () => {
  ...
  it("Cancel closes the sheet — full undo; played-starter token returns to its tappable state", () => {
    ...
    const token = screen.getByTitle(/DEF1.*played — tap to bench \(forfeits 7 pts\)/i) as HTMLButtonElement;
    expect(token.disabled).toBe(false);
  });
