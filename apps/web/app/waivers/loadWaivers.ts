...
export async function loadWaivers(viewerManagerId: string): Promise<WaiversView | null> {
  ...
  const [league, managerRows, pendingBids, rosterRows, ownedRows, upcomingMatches, seasonScores] =
    await Promise.all([
      prisma.league.findUnique({
        where: { id: leagueId },
        select: { faabBatchLocalTime: true, timezone: true, status: true },
      }),
  ...
  return {
    ...
    batches,
    waiverOrder,
    batchLocalTime: league?.faabBatchLocalTime ?? "06:00",
