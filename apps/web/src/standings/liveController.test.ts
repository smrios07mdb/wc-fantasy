...
import { startStandingsLive } from "./liveController";
import type { RealtimeChannelLike, RealtimeClientLike } from "./realtime";

const view = (id: string) => ({ meId: id }) as unknown as StandingsView;
const flush = async () => {
...
describe("startStandingsLive — token gate, seq-guarded refetch, visibility-gated poll, teardown", () => {
  it("is a no-op (no subscribe) without an access token — an anon socket gets zero RLS-gated changes", () => {
    const f = fakeClient();
    const stop = startStandingsLive({
      client: f.client,
      accessToken: null,
  ...
  it("seq-guards concurrent refetches: only the LATEST response is applied", async () => {
    const f = fakeClient();
    const onSnapshot = vi.fn();
    const resolvers: ((v: StandingsView) => void)[] = [];
    startStandingsLive({
      client: f.client,
      accessToken: "jwt",
      fetchSnapshot: () => new Promise<StandingsView>((res) => resolvers.push(res)),
      onSnapshot,
      timers: fakeTimers().timers,
    });
