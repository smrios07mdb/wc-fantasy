 ...
 *
 * HOME (apps/web/src/kit/) — this is the NEUTRAL, surface-agnostic home of the kit primitive. It was
 * moved here out of app/vsfield/ so no single route "owns" it: vsfield + game-detail import it via the
 * `app/vsfield/kitOf.ts` re-export shim (byte-compatible), and lineup imports it directly from here.
 * There is NO name-keyed `JERSEY_BG` map in apps/web — that was a design-prototype construct only; the
 * app keys kits by ISO-2 through `kitOf()`. Do not re-introduce a name-keyed map.
 */
