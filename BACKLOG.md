...
- T11 — DONE (held). The box-score path (`PlayerScoreSheet` → `/api/player-box` → `loadPlayerBox`) was already period-aware and needed NO change; only period SELECTION was missing. Added a shared pure `apps/web/src/period/selectablePeriods.ts` (started-set = `isPickLocked` on `matches[0]`, `periodIsDone` = last kickoff + window, `resolveDisplayedPeriodId` = server-side future-rejection) consumed by all three loaders. Lineup extends `PeriodTabs` (prior periods the manager played, read-only, default pinned to the live wave); vsfield threads `?period=` → `loadVsField(periodId)` (live sub suppressed for a prior); waivers adds a client selector that swaps the drill-down to `PlayerScoreSheet` for a prior matchday (FA pool/claims/batch stay live/global).
