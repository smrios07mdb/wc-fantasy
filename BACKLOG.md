...
|----|------|-------|--------|------|--------|-------|
| **T5/T6** | Game detail — click a game (dashboard + own Games tab) → both lineups + every player's live pts | High | L | contained (new loader; review loader) | `TODO` | commish re-confirmed 2026-06-21 |
...
- T5/T6 — per-player fantasy points already computed by the box-score path (`PlayerScoreSheet` / `score_player_match`). New work is a match→both-lineups loader.
...
1. **T5/T6** — match/game detail + dashboard game-click.
...
- **T5** — screenshot text was truncated; confirm the full intent when the thread opens.
