 ...
 * Prisma/Supabase"). The resolver, validator, controller, store ports, error vocabulary, and the
 * memory doubles must carry NO IO: no Prisma / @app/db, no Supabase, no Next, no env, no clock, no
 * fetch. (`now` and kickoffs are always injected.) The Prisma adapter lives in `prismaStore.ts`, which
 * is deliberately EXCLUDED from this list and reachable only via `@app/faab/prisma`.
 *
