...
describe("STAT_DIRTY_UPDATE (shared no-clobber invariant)", () => {
  it("the CONFLICT/update branch flips ONLY `dirty` — never a stat column", () => {
    // Shared by @app/ingest (05a event re-mark) and @app/scrape (05b scrape rating): a late write must
    // re-dirty the player WITHOUT nulling stats that already landed. Nulls live only in the insert branch.
