...
function DesktopApp({ field, season, feed, freshIds, t, conn, view, setView, selected, setSelected, activeMatch, setActiveMatch, ko, ceremony, setCeremony }){
  const lastUpd = conn==='stale' ? '1:34 ago' : 'just now';
  const pool = ko ? ko.alive : field.ranked;
  const myIdx = pool.findIndex(s=>s.id===ME_ID);
  const defRival = (pool[myIdx-1] || pool[myIdx+1] || pool.find(s=>s.id!==ME_ID));
    ...
    <div className="v2-app">
      <div className="v2-top">
        <div className="v2-top-brand">
          ...
          <div>
            <b className="display">{ko?'The Cut':'Vs the Field'}</b>
            <div className="t-micro text-tertiary">{ko ? (KO_ROUND.label+' · guillotine').toUpperCase() : PERIOD.label.toUpperCase()+' · '+PERIOD.sub.toUpperCase()}</div>
          </div>
        </div>
      ...
      {view==='season'
        ...
        : <>
            ...
            <DirectionA field={field} t={t} conn={conn} effSel={effSel} setSelected={setSelected} ko={ko} />
            <FeedTicker feed={feed} freshIds={freshIds} />
          </>}
      {ko && ceremony && <KOCeremony ko={ko} onClose={()=>setCeremony(false)} />}
    </div>
  );
}
...
function App(){
  ...
  const ko = useMemo(()=> phase==='knockout' ? knockoutContext(field, t, +(tw.cut||2)) : null, [field, t, phase, tw.cut]);
  ...
  const shared = { field, season, feed, freshIds, t, conn, view, setView, selected, setSelected, activeMatch, setActiveMatch, ko, ceremony, setCeremony };
    ...
    <div className="vf-stage">
      <div className="vf-stagebar">
        <div className="vf-sb-title">
          <div className="vf-logo">W</div>
          <div><b className="display" style={{fontSize:15,lineHeight:1}}>{phase==='knockout'?'The Cut — knockout live surface':'Vs the Field — live surface'}</b>
        ...
        <div className="vf-sb-sim">
          ...
          <div className="vf-sb-jumps">
            ...
            <button className={'btn btn-sm '+(phase==='knockout'?'btn-primary':'btn-ghost')} onClick={()=>{ setPhase(p=>p==='knockout'?'group':'knockout'); setSelected(null); setCeremony(false); }} title="Toggle the guillotine knockout phase">⚔ Knockout</button>
            <button className="btn btn-ghost btn-sm" disabled={phase!=='knockout'} onClick={()=>setCeremony(true)} title="Play the cutting ceremony (results official)">🪓 Blade drops</button>
          </div>
        </div>
      ...
      <TweaksPanel>
        ...
        <TweakSection label="Knockout — commissioner-set" />
        <TweakRadio label="Cut this round" value={tw.cut||'2'} options={['2','3']} onChange={v=>setTweak('cut', v)} />
      </TweaksPanel>
    </div>
  );
}
