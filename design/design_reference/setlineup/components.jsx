...
function PitchToken({ cell, t, tokenStyle, state, onTap, drag, onScore }) {
  ...
  if (tokenStyle==='named') {
    return (
      <button className={cls} onClick={()=>onTap(cell)} {...dragProps} {...dropProps}>
  ...
  if (tokenStyle==='disc') {
    return (
      <button className={cls} onClick={()=>onTap(cell)} {...dragProps} {...dropProps} title={shortName(p)}>
...
function BenchRow({ cell, t, ix }) {
  ...
  return (
    <button className={'sl-bench-row s-'+status+' st-'+state} onClick={()=>ix.onCellTap(cell)} {...dragProps} {...dropProps}>
      <Pos p={p.pos}/>
      <Flag nat={p.nat}/>
...
function XIList({ lineup, t, ix }) {
  ...
  LANES.forEach(pos => (lineup.slots[pos]||[]).forEach((id,idx)=>{
    ...
    rows.push(
      <button key={pos+idx} className={'sl-xi-row s-'+status+' st-'+state} onClick={()=>ix.onCellTap(cell)}>
        <Pos p={pos}/>
    ...
    <div className="sl-sheet-scrim" onClick={onClose}>
      <div className="sl-scoremodal" onClick={ev=>ev.stopPropagation()}>
        ...
        {e.done.length===0
          ...
          : <div className="sl-sm-list">
              {e.done.map((ev,i)=>(
                <div className="sl-sm-row" key={i}>
                  <span><span className="mono text-tertiary" style={{marginRight:8}}>{ev.min}'</span>{ev.label}</span>
                  <span className={'sl-sm-pts '+(ev.pts>=0?'is-pos':'is-neg')}>{ev.pts>=0?'+':''}{ev.pts}</span>
                </div>
              ))}
            </div>}
