...
function MClaim({ bid, t, idx, count, onEdit, onCancel, onReorder }){
...
function MobileWaivers(props){
  const { tab, setTab, bids, t, st, priority, phase, onAdd, onEdit, onCancel, onReorder, layout, conn, theme } = props;
          ...
          {tab==='claims' ? (
            ...
            : <>
              ...
              {bids.length===0
                ...
                : bids.map((b,i)=><MClaim key={b.id} bid={b} t={t} idx={i} count={bids.length} onEdit={onEdit} onCancel={onCancel} onReorder={onReorder}/>)}
            </>
          ) : (
            <div className="mwv-results">{HISTORY.map(b=><ResultsBatch key={b.id} batch={b} layout={layout}/>)}</div>
          )}
        </div>
      </div>
    </IOSDevice>
