...
function ClaimRow({ bid, t, idx, count, onEdit, onCancel, onReorder, onPlayer }){
...
// ---------- results history ----------
function ResultItem({ r, layout, onPlayer }){
  const w = mgr(r.winnerId);
  const oc = r.outcome; // won | lost | void
  return (
    <div className={'wv-res'+(r.mine?' is-mine':'')+(layout==='table'?' is-table':'')}>
      <button type="button" className="wv-res-add wv-padd" onClick={()=>onPlayer&&onPlayer(r.add)} title="View player card">
...
function ResultsBatch({ batch, layout, onPlayer }){
  return (
    <section className="wv-batch">
      ...
      <div className={'wv-batch-list'+(layout==='table'?' is-table':'')}>
        {batch.results.map((r,i)=><ResultItem key={i} r={r} layout={layout} onPlayer={onPlayer}/>)}
      </div>
    </section>
  );
}
