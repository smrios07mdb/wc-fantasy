...
// ---- leaderboard row ----
function MaRow({ snap, myTotal, onTap, dimLive }){
  const m = mgr(snap.id);
  const d = myTotal - snap.total;
  const k = d>0?'W':d<0?'L':'D';
  const pulse = useScorePulse(snap.total);
  return (
    <button className={'ma-row'+(m.isMe?' is-me':'')} onClick={()=>onTap(snap.id)}>
...
function MobileVsFieldA({ t, conn, view, setView, field, season, feed, freshIds, selected, setSelected, activeMatch, setActiveMatch }){
  const me = field.snaps.find(s=>s.id===ME_ID);
      ...
      <div className="ma" data-theme="dark">
        <div className="ma-head">
          <div className="ma-headrow">
            <div><div className="ma-title">Vs the Field</div>
        ...
        <div className="ma-scroll">
          {view==='season' ? <MaSeason season={season} />
            ...
            : <>
                ...
                <button className="ma-fieldbtn" onClick={()=>setSelected('field')}>
                ...
                <div className="ma-listlab"><span className="t-label">Standings · tap to compare</span><span className="t-micro text-tertiary">live · pts</span></div>
                <div className="ma-list">
                  {field.ranked.map(s => <MaRow key={s.id} snap={s} myTotal={me.total} onTap={setSelected} dimLive={dimLive} />)}
                </div>
