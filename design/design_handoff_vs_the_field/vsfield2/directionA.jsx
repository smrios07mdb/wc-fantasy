...
// ============================================================ leaderboard ===
function LbRow({ snap, myTotal, selected, onSelect, dimLive }){
  const m = mgr(snap.id);
  const d = myTotal - snap.total;
  const k = d>0?'W':d<0?'L':'D';
  const pulse = useScorePulse(snap.total);
  return (
    <button className={'da-lb-row'+(m.isMe?' is-me':'')+(selected?' is-sel':'')} onClick={()=>onSelect(snap.id)}>
...
function Leaderboard({ field, effSel, onSelect, dimLive }){
  const me = field.snaps.find(s=>s.id===ME_ID);
  return (
    <div className="da-lb">
      <button className={'da-lb-fieldbtn'+(effSel==='field'?' is-sel':'')} onClick={()=>onSelect('field')} title="You vs the whole field">
      ...
      <div className="da-lb-head"><span className="t-label">Standings</span><span className="t-micro text-tertiary">live · pts</span></div>
      {field.ranked.map(s => <LbRow key={s.id} snap={s} myTotal={me.total} selected={effSel===s.id} onSelect={onSelect} dimLive={dimLive} />)}
    </div>
  );
}
...
function XIToken({ r, onScore, dimLive }){
  ...
  return (
    <button className={'sl-tok sl-tok-jersey '+sCls+(dimLive?' is-dim':'')} onClick={()=>onScore(r.p)}
      ...
      {r.status==='ytp'
        ? <span className="sl-tok-toplay">to play</span>
        : <span className={'sl-tok-score s-'+r.status+(r.pts===0?' is-zero':'')}>
            {r.status==='live' && !dimLive && <span className="sl-score-dot"></span>}<b>{r.pts}</b> pts
          </span>}
    </button>
  );
}
...
// ============================================================ body ===
function DirectionA({ field, t, conn, effSel, setSelected }){
  const me = field.snaps.find(s=>s.id===ME_ID);
  ...
  return (
    <div className="da-body">
      <Leaderboard field={field} effSel={effSel} onSelect={setSelected} dimLive={dimLive} />
