...
function App(){
  ...
  const [scored, setScored] = useState(null);   // { p, managerId } → open player breakdown
  ...
  const openScore = useCallback((p, managerId)=> setScored({ p, managerId }), []);
  const closeScore = useCallback(()=> setScored(null), []);

  const sharedProps = { t, conn, view, setView, field, season, feed, freshIds, selected, setSelected, activeMatch, setActiveMatch, scored, openScore, closeScore };
