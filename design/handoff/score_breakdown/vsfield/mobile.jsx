...
function MobH2H({ field, oppId, onBack, openScore }) {
  ...
  const list = (snap) => [...snap.rows].sort((a,b)=>(order[a.status]-order[b.status])||(b.pts-a.pts)).map((r,i)=>(
    <button className={'mvf-plrow vf-statusrow-'+r.status} key={i} onClick={()=>openScore(r.p, snap.id)}>
      ...
      <div className="mvf" data-theme="dark">
        ...
        <div className="mvf-scroll">
          {view==='season' ? <MobSeason season={season} />
            : selected ? <MobH2H field={field} oppId={selected} onBack={()=>setSelected(null)} openScore={openScore} />
